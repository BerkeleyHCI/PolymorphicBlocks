...
class BomItem:
    def __init__(self, designator, footprint, value, manufacturer, part_number, datasheet):
    ...
    def list_values(self) -> List[str]:       # makes it easier to populate .csv file
        return [str(self.designator), str(self.value), str(self.footprint),
                str(self.manufacturer), str(self.part_number), str(self.datasheet)]
...
class BomBackend(BaseBackend):      # creates and populates .csv file
    def run(self, design: CompiledDesign) -> List[Tuple[edgir.LocalPath, str]]:
        ...
        with open('BOM.csv', 'w', newline='') as f:
            fieldnames = ['Ref Des', 'Value', 'Footprint', 'Manufacturer', 'Part Number', 'Datasheet']
            thewriter = csv.DictWriter(f, fieldnames=fieldnames)

            thewriter.writeheader()      # creates the header

            for item in bom_list:
                thewriter.writerow({'Ref Des' : item.designator, 'Value' : item.value,
...
class BomTransform(TransformUtil.Transform):
    ...
    def visit_block(self, context: TransformUtil.TransformContext, block: edgir.BlockTypes) -> None:
        bom = BomItem(str(self.design.get_value(context.path.to_tuple() + ('fp_refdes',))),
