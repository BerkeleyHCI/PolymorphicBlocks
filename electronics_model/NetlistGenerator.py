...
class NetlistTransform(TransformUtil.Transform):
    ...
    def process_blocklike(self, path: TransformUtil.Path,
                          block: Union[edgir.Link, edgir.LinkArray, edgir.HierarchyBlock]) -> None:
    ...
    def connect_ports(self, elt1: Tuple[TransformUtil.Path, edgir.EltTypes],
        ...
        def pin_name_goodness(pin1: TransformUtil.Path, pin2: TransformUtil.Path) -> int:
            ...
            if pin1.links and pin1.links[0].startswith('anon') and \
                ...
                return len(pin1.blocks) - len(pin2.blocks)
                    ...
                    (len(pin2.ports) != 1 or (
                            pin2.ports and not pin2.ports[-1].isnumeric())):  # disprefer number-only ports
                return 1
            elif len(pin2.ports) == 1 and pin2.ports[0].isnumeric and \
                    (len(pin1.ports) != 1 or (
                            pin1.ports and not pin1.ports[-1].isnumeric())):  # disprefer number-only ports
                return -1
