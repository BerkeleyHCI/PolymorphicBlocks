...
from .KiCadSchematicBlock import parse_resistor, parse_capacitor


class KicadPartParsingTest(unittest.TestCase):
  def test_resistor(self):
    self.assertEqual(parse_resistor("51"), Range.from_tolerance(51, 0.05))
  ...
  def test_capacitor(self):
    self.assertEqual(parse_capacitor("0.1uF 6.3V"), (Range.from_tolerance(0.1e-6, 0.20),
                                                     Range.zero_to_upper(6.3)))
    self.assertEqual(parse_capacitor("4.7u 6.3V"), (Range.from_tolerance(4.7e-6, 0.20),
                                                    Range.zero_to_upper(6.3)))
    self.assertEqual(parse_capacitor("0.1uF 10% 12V"), (Range.from_tolerance(0.1e-6, 0.10),
