...
class ResistorMux(Interface, KiCadImportableBlock, GeneratorBlock):
    ...
    def symbol_pinning(self, symbol_name: str) -> Dict[str, BasePort]:
        ...
        return {"control": self.control, "sw": self.com, "com": self.input, "V+": self.pwr, "V-": self.gnd}
...
class Multimeter(JlcBoardTop):
    ...
    def contents(self) -> None:
        ...
        with self.implicit_connect(
            ...
            (self.gate, self.reg_5v, self.tp_5v, self.prot_5v, self.reg_3v3, self.tp_3v3, self.prot_3v3), _ = (
                self.chain(
                    self.vbat,
        ...
        with self.implicit_connect(
            ...
            (self.vbatsense,), _ = self.chain(
                self.gate.pwr_out,  # TODO update to use VoltageSenseDivider
        ...
        with self.implicit_connect(
            ...
            (self.spk_pwr,), _ = self.chain(
                self.v5v, self.Block(ForcedVoltageCurrentDraw((0, 0.05) * Amp)), self.spk_drv.pwr
            )
        ...
        with self.implicit_connect(
            ...
            (self.measure_buffer, self.tp_measure), self.meas_chain = self.chain(
                self.measure.output, imp.Block(OpampFollower()), self.Block(AnalogTestPoint())
            )
            (self.adc,), _ = self.chain(imp.Block(Mcp3561()), adc_spi)
            ...
            # DRIVER CIRCUITS
            self.driver = imp.Block(MultimeterCurrentDriver(voltage_rating=VOLTAGE_RATING))
