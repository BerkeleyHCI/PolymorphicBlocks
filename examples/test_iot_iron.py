...
class IronConnector(Connector, Block):
    ...
    def __init__(
        self, *, isense_resistance: RangeLike = 22 * mOhm(tol=0.05), current_draw: RangeLike = (0, 3.25) * Amp
    ):
        super().__init__()
        ...
        self.conn = self.Block(PinHeader254(3)).connected({"2": self.pwr, "3": self.thermocouple})
    ...
    def contents(self) -> None:
        ...
        with self.implicit_connect(
            ...
            (self.reg_3v3, self.tp_3v3, self.prot_3v3), _ = self.chain(
                self.vusb,
                imp.Block(BuckConverter(output_voltage=3.3 * Volt(tol=0.05), input_ripple_limit=100 * mVolt)),
        ...
        with self.implicit_connect(
            ...
            (self.spk_drv, self.spk), _ = self.chain(
                self.mcu.with_mixin(IoControllerI2s()).i2s.request("spk"), imp.Block(Max98357a()), self.Block(Speaker())
            )
        ...
        with self.implicit_connect(
            ...
            (self.conv_force, self.conv, self.tp_conv), _ = self.chain(
                ...
                imp.Block(
                    CustomSyncBuckConverterIndependent(
                        output_voltage=(5, 5) * Volt,
                        frequency=200 * kHertz(tol=0),
            ...
            (self.low_pull, self.low_rc), _ = self.chain(
                self.mcu.gpio.request("pwm_low"), imp.Block(pull_model), imp.Block(rc_model), self.conv.pwm_low
            )
            (self.high_pull, self.high_rc), _ = self.chain(
                self.mcu.gpio.request("pwm_high"), imp.Block(pull_model), imp.Block(rc_model), self.conv.pwm_high
            )
        ...
        with self.implicit_connect(
            ...
            self.tamp = imp.Block(DifferentialAmplifier(ratio=(150, 165), input_impedance=(0.9, 5) * kOhm))
    ...
    def refinements(self) -> Refinements:
        return super().refinements() + Refinements(
            ...
            instance_values=[
                ...
                (
                    ["iron", "isense_res", "res", "res", "filter_footprints"],
                ...
                (
                    ["conv", "power_path", "inductor", "manual_frequency_rating"],
                    Range(0, 1e6),
                ),  # from charts, inductance constant up to 1MHz
