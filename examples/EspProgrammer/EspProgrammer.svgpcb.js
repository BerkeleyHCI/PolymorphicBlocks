const board = new PCB();

// jlc_th.th1
const UH1 = board.add(JlcToolingHole_1_152mm, {
  translate: pt(0.000, 1.128), rotate: 0,
  id: 'UH1'
})
// jlc_th.th2
const UH2 = board.add(JlcToolingHole_1_152mm, {
  translate: pt(0.039, 1.128), rotate: 0,
  id: 'UH2'
})
// jlc_th.th3
const UH3 = board.add(JlcToolingHole_1_152mm, {
  translate: pt(0.000, 1.167), rotate: 0,
  id: 'UH3'
})
// usb_uart.conn
const UJ1 = board.add(USB_C_Receptacle_XKB_U262_16XN_4BVC11, {
  translate: pt(0.857, 0.165), rotate: 0,
  id: 'UJ1'
})
// usb_uart.esd
const UU1 = board.add(SOT_23, {
  translate: pt(0.723, 0.458), rotate: 0,
  id: 'UU1'
})
// usb_uart.cc_pull.cc1
const UR1 = board.add(R_0402_1005Metric, {
  translate: pt(0.874, 0.409), rotate: 0,
  id: 'UR1'
})
// usb_uart.cc_pull.cc2
const UR2 = board.add(R_0402_1005Metric, {
  translate: pt(0.987, 0.409), rotate: 0,
  id: 'UR2'
})
// vusb_protect.diode
const UD1 = board.add(D_SOD_323, {
  translate: pt(1.369, 0.797), rotate: 0,
  id: 'UD1'
})
// usbconv.ic
const UU2 = board.add(QFN_28_1EP_5x5mm_P0_5mm_EP3_35x3_35mm, {
  translate: pt(0.485, 0.881), rotate: 0,
  id: 'UU2'
})
// usbconv.regin_cap0.cap
const UC1 = board.add(C_0402_1005Metric, {
  translate: pt(0.682, 0.778), rotate: 0,
  id: 'UC1'
})
// usbconv.regin_cap1.cap
const UC2 = board.add(C_0402_1005Metric, {
  translate: pt(0.682, 0.853), rotate: 0,
  id: 'UC2'
})
// usbconv.vdd_cap.cap
const UC3 = board.add(C_0402_1005Metric, {
  translate: pt(0.682, 0.929), rotate: 0,
  id: 'UC3'
})
// reg_3v3.ic
const UU3 = board.add(SOT_23_5, {
  translate: pt(0.081, 0.826), rotate: 0,
  id: 'UU3'
})
// reg_3v3.in_cap.cap
const UC4 = board.add(C_0402_1005Metric, {
  translate: pt(0.209, 0.951), rotate: 0,
  id: 'UC4'
})
// reg_3v3.out_cap.cap
const UC5 = board.add(C_0805_2012Metric, {
  translate: pt(0.067, 0.971), rotate: 0,
  id: 'UC5'
})
// out.conn
const UJ2 = board.add(PinHeader_2x03_P2_54mm_EdgeInline, {
  translate: pt(0.159, 0.321), rotate: 0,
  id: 'UJ2'
})
// auto.q_en
const UQ1 = board.add(SOT_323_SC_70, {
  translate: pt(1.251, 0.051), rotate: 0,
  id: 'UQ1'
})
// auto.q_boot
const UQ2 = board.add(SOT_323_SC_70, {
  translate: pt(1.251, 0.193), rotate: 0,
  id: 'UQ2'
})
// auto.dtr_res
const UR3 = board.add(R_0402_1005Metric, {
  translate: pt(1.394, 0.160), rotate: 0,
  id: 'UR3'
})
// auto.rts_res
const UR4 = board.add(R_0402_1005Metric, {
  translate: pt(1.221, 0.302), rotate: 0,
  id: 'UR4'
})
// led.package
const UD2 = board.add(LED_0603_1608Metric, {
  translate: pt(0.895, 0.788), rotate: 0,
  id: 'UD2'
})
// led.res
const UR5 = board.add(R_0402_1005Metric, {
  translate: pt(0.873, 0.875), rotate: 0,
  id: 'UR5'
})
// led_en.package
const UD3 = board.add(LED_0603_1608Metric, {
  translate: pt(1.130, 0.788), rotate: 0,
  id: 'UD3'
})
// led_en.res
const UR6 = board.add(R_0402_1005Metric, {
  translate: pt(1.108, 0.875), rotate: 0,
  id: 'UR6'
})

board.setNetlist([
  {name: "vusb", pads: [["UJ1", "A4"], ["UJ1", "A9"], ["UJ1", "B4"], ["UJ1", "B9"], ["UD1", "1"], ["UU2", "7"], ["UU2", "8"], ["UC1", "1"], ["UC2", "1"], ["UU3", "1"], ["UU3", "3"], ["UC4", "1"]]},
  {name: "gnd", pads: [["UJ1", "A1"], ["UJ1", "A12"], ["UJ1", "B1"], ["UJ1", "B12"], ["UJ1", "S1"], ["UU1", "3"], ["UR1", "1"], ["UR2", "1"], ["UD1", "2"], ["UU2", "29"], ["UU2", "3"], ["UC1", "2"], ["UC2", "2"], ["UC3", "2"], ["UU3", "2"], ["UC4", "2"], ["UC5", "2"], ["UJ2", "5"]]},
  {name: "v3v3", pads: [["UU3", "5"], ["UC5", "1"], ["UJ2", "1"], ["UD2", "2"], ["UD3", "2"]]},
  {name: "usb_uart.usb.dp", pads: [["UJ1", "A6"], ["UJ1", "B6"], ["UU1", "2"], ["UU2", "4"]]},
  {name: "usb_uart.usb.dm", pads: [["UJ1", "A7"], ["UJ1", "B7"], ["UU1", "1"], ["UU2", "5"]]},
  {name: "usb_uart.conn.cc.cc1", pads: [["UJ1", "A5"], ["UR1", "2"]]},
  {name: "usb_uart.conn.cc.cc2", pads: [["UJ1", "B5"], ["UR2", "2"]]},
  {name: "usbconv.suspend", pads: [["UU2", "12"], ["UR5", "2"]]},
  {name: "usbconv.nsuspend", pads: [["UU2", "11"]]},
  {name: "usbconv.uart.tx", pads: [["UU2", "26"], ["UJ2", "4"]]},
  {name: "usbconv.uart.rx", pads: [["UU2", "25"], ["UJ2", "3"]]},
  {name: "usbconv.ri", pads: [["UU2", "2"]]},
  {name: "usbconv.dcd", pads: [["UU2", "1"]]},
  {name: "usbconv.dtr", pads: [["UU2", "28"], ["UQ2", "2"], ["UR3", "1"]]},
  {name: "usbconv.dsr", pads: [["UU2", "27"]]},
  {name: "usbconv.rts", pads: [["UU2", "24"], ["UQ1", "2"], ["UR4", "1"], ["UR6", "2"]]},
  {name: "usbconv.cts", pads: [["UU2", "23"]]},
  {name: "usbconv.ic.vdd", pads: [["UU2", "6"], ["UU2", "9"], ["UC3", "1"]]},
  {name: "out.en", pads: [["UJ2", "6"], ["UQ1", "3"]]},
  {name: "out.boot", pads: [["UJ2", "2"], ["UQ2", "3"]]},
  {name: "auto.q_en.base", pads: [["UQ1", "1"], ["UR3", "2"]]},
  {name: "auto.q_boot.base", pads: [["UQ2", "1"], ["UR4", "2"]]},
  {name: "led.package.k", pads: [["UD2", "1"], ["UR5", "1"]]},
  {name: "led_en.package.k", pads: [["UD3", "1"], ["UR6", "1"]]}
])

const limit0 = pt(-0.07874015748031496, -0.07874015748031496);
const limit1 = pt(1.5503937007874018, 1.2854330708661417);
const xMin = Math.min(limit0[0], limit1[0]);
const xMax = Math.max(limit0[0], limit1[0]);
const yMin = Math.min(limit0[1], limit1[1]);
const yMax = Math.max(limit0[1], limit1[1]);

const filletRadius = 0.1;
const outline = path(
  [(xMin+xMax/2), yMax],
  ["fillet", filletRadius, [xMax, yMax]],
  ["fillet", filletRadius, [xMax, yMin]],
  ["fillet", filletRadius, [xMin, yMin]],
  ["fillet", filletRadius, [xMin, yMax]],
  [(xMin+xMax/2), yMax],
);
board.addShape("outline", outline);

renderPCB({
  pcb: board,
  layerColors: {
    "F.Paste": "#000000ff",
    "F.Mask": "#000000ff",
    "B.Mask": "#000000ff",
    "componentLabels": "#00e5e5e5",
    "outline": "#002d00ff",
    "padLabels": "#ffff99e5",
    "B.Cu": "#ef4e4eff",
    "F.Cu": "#ff8c00cc",
  },
  limits: {
    x: [xMin, xMax],
    y: [yMin, yMax]
  },
  background: "#00000000",
  mmPerUnit: 25.4
})


