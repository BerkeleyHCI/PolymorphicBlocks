...
class RobotOwl(JlcBoardTop):
    ...
    def contents(self) -> None:
        ...
        (self.reg_12v, self.tp_12v), _ = self.chain(
            self.vusb, self.Block(BoostConverter(output_voltage=(12, 15) * Volt)), self.Block(VoltageTestPoint())
        )
        ...
        with self.implicit_connect(
            ...
            (self.photodiode,), _ = self.chain(imp.Block(PhotodiodeSensor()), self.mcu.adc.request("photodiode"))
        ...
        with self.implicit_connect(
            ...
            (self.spk_drv, self.spk), _ = self.chain(
                mcu_i2s.i2s.request("speaker"), imp.Block(Max98357a()), self.Block(Speaker())
            )

            self.servo = ElementDict[PwmConnector]()
            for i in range(2):
                (self.servo[i],), _ = self.chain(
                    self.mcu.gpio.request(f"servo{i}"), imp.Block(PwmConnector((0, 200) * mAmp))
                )

            (self.ws2812bArray, self.extNeopixels), _ = self.chain(
                self.mcu.gpio.request("ws2812"), imp.Block(NeopixelArray(12)), imp.Block(LedConnector())
            )
    ...
    def refinements(self) -> Refinements:
        return super().refinements() + Refinements(
            ...
            instance_values=[
                ...
                (
                    ["photodiode", "out", "signal_out"],
                    Range(0, 2.2),
                ),  # discard the extra range to make it ESP compatible
