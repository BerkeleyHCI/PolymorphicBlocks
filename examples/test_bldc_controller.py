...
class BldcConnector(Connector, Block):
    ...
    def __init__(self, max_current: FloatLike):
        ...
        phase_model = DigitalSink(current_draw=(-max_current, max_current))
        for i in ["1", "2", "3"]:
...
class BldcController(JlcBoardTop):
    ...
    def contents(self) -> None:
        ...
        with self.implicit_connect(
            ...
            (self.hall_pull, self.hall_tp), _ = self.chain(
                self.hall.phases,
        ...
        with self.implicit_connect(
            ...
            self.vsense = imp.Block(
                VoltageSenseDivider(full_scale_voltage=(3.0, 3.3) * Volt, impedance=10 * kOhm(tol=0.2))
            )
            self.connect(self.motor_pwr.pwr, self.vsense.input)
            (self.vsense_tp,), _ = self.chain(
                self.vsense.output, self.Block(AnalogTestPoint()), self.mcu.adc.request("vsense")
            )

            self.isense = imp.Block(
                OpampCurrentSensor(
            ...
            (self.bldc_fault_tp,), _ = self.chain(
                self.mcu.gpio.request("bldc_fault"), self.Block(DigitalTestPoint()), self.bldc_drv.nfault
            )
            (self.bldc_en_tp,), _ = self.chain(
                self.mcu.gpio.request_vector("bldc_en"), self.Block(DigitalArrayTestPoint()), self.bldc_drv.ens
            )
            (self.bldc_in_tp,), _ = self.chain(
                self.mcu.gpio.request_vector("bldc_in"), self.Block(DigitalArrayTestPoint()), self.bldc_drv.ins
            )
            ...
            for i in ["1", "2", "3"]:
                ...
                (
                    ...
                    self.bldc_drv.pgnd_sense.request(i),
                    self.curr_amp[i],
                    self.Block(AnalogTestPoint()),
    ...
    def refinements(self) -> Refinements:
        return super().refinements() + Refinements(
            instance_refinements=[(["mcu"], Feather_Nrf52840), (["isense", "amp", "amp"], Opa197)],
            instance_values=[
                (
                    ["mcu", "pin_assigns"],
                    [
