...
class Lf21215tmr_Device(FootprintBlock):
    def __init__(self) -> None:
        super().__init__()
        self.vcc = self.Port(VoltageSink(voltage_limits=(1.8, 5.5) * Volt, current_draw=(0.5, 2.0) * uAmp))
        self.gnd = self.Port(Ground())

        self.vout = self.Port(
            DigitalSource.from_supply(
                self.gnd, self.vcc, current_limits=(-9, 9) * mAmp, output_threshold_offset=(0.2, -0.3)
            )
        )
    ...
    def contents(self) -> None:
        ...
        self.import_kicad(
            ...
            conversions={
                ...
                ),  # TODO: also model draw of external bridge?
                "gnd": Ground(),
