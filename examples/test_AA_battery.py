...
class AABatteryCircuit(SimpleBoardTop):
  def contents(self) -> None:
    ...
    self.li_ion_bat = self.Block(AABattery())
    ...
    self.link_bat_neg = self.connect(self.li_ion_bat.gnd, self.boost.gnd)
    self.link_bat_pos = self.connect(self.li_ion_bat.pwr, self.boost.pwr_in)
