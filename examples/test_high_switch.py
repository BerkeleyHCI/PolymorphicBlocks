...
class CalSolPowerConnector(Connector, FootprintBlock):
    ...
    def contents(self) -> None:
        ...
        self.footprint(
            "J",
            "calisco:Molex_DuraClik_vert_3pin",
            {
                "1": self.gnd,
                "2": self.pwr,
                "3": self.gnd,
...
class CalSolCanConnector(Connector, FootprintBlock):
    ...
    def contents(self) -> None:
        ...
        self.footprint(
            "J",
            "calisco:Molex_DuraClik_vert_5pin",
            {
                # 1 is SHLD
                "2": self.pwr,
                "3": self.gnd,
...
class CalSolCanConnectorRa(Connector, FootprintBlock):
    ...
    def contents(self) -> None:
        ...
        self.footprint(
            "J",
            "calisco:Molex_DuraClik_502352_1x05_P2.00mm_Horizontal",
            {
                # 1 is SHLD
                "2": self.pwr,
                "3": self.gnd,
...
class M12CanConnector(Connector, FootprintBlock):
    ...
    def contents(self) -> None:
        ...
        self.footprint(
            "J",
            "calisco:PhoenixContact_M12-5_Pin_SACC-DSIV-MS-5CON-L90",
            {
                # 1 is SHLD
                "2": self.pwr,
                "3": self.gnd,
...
class LightsConnector(Connector, FootprintBlock):
    ...
    def contents(self) -> None:
        ...
        self.footprint(
            "J",
            "calisco:Molex_DuraClik_vert_4pin",
            {
                "1": self.pwr,
                "2": self.out[0],
                "3": self.out[1],
...
class HighSwitch(BoardTop):
    ...
    def contents(self) -> None:
        ...
        with self.implicit_connect(
            ...
            (self.pwr,), _ = self.chain(self.vin, imp.Block(Tps561201(output_voltage=3.3 * Volt(tol=0.05))))
        ...
        with self.implicit_connect(
            ...
            (self.can,), self.can_chain = self.chain(
    ...
    def refinements(self) -> Refinements:
        return super().refinements() + Refinements(
            ...
            instance_values=[
                ...
                (["light[0]", "drv[0]", "drv", "filter_footprints"], ["Package_TO_SOT_SMD:TO-252-2"]),
                (
                    ["light[0]", "drv[1]", "drv", "filter_footprints"],
                    ParamValue(["light[0]", "drv[0]", "drv", "filter_footprints"]),
                ),
                (
                    ["light[1]", "drv[0]", "drv", "filter_footprints"],
                    ParamValue(["light[0]", "drv[0]", "drv", "filter_footprints"]),
                ),
                (
                    ["light[1]", "drv[1]", "drv", "filter_footprints"],
                    ParamValue(["light[0]", "drv[0]", "drv", "filter_footprints"]),
                ),
                (
                    ["light[2]", "drv[0]", "drv", "filter_footprints"],
                    ParamValue(["light[0]", "drv[0]", "drv", "filter_footprints"]),
                ),
                (
                    ["light[2]", "drv[1]", "drv", "filter_footprints"],
                    ParamValue(["light[0]", "drv[0]", "drv", "filter_footprints"]),
                ),
                (
                    ["light[3]", "drv[0]", "drv", "filter_footprints"],
                    ParamValue(["light[0]", "drv[0]", "drv", "filter_footprints"]),
                ),
                (
                    ["light[3]", "drv[1]", "drv", "filter_footprints"],
                    ParamValue(["light[0]", "drv[0]", "drv", "filter_footprints"]),
                ),
                (
                    ["light[4]", "drv[0]", "drv", "filter_footprints"],
                    ParamValue(["light[0]", "drv[0]", "drv", "filter_footprints"]),
                ),
                (
                    ["light[4]", "drv[1]", "drv", "filter_footprints"],
                    ParamValue(["light[0]", "drv[0]", "drv", "filter_footprints"]),
                ),
                (
                    ["light[5]", "drv[0]", "drv", "filter_footprints"],
                    ParamValue(["light[0]", "drv[0]", "drv", "filter_footprints"]),
                ),
                (
                    ["light[5]", "drv[1]", "drv", "filter_footprints"],
                    ParamValue(["light[0]", "drv[0]", "drv", "filter_footprints"]),
                ),
            ],
