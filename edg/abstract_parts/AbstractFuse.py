...
class Fuse(DiscreteComponent, HasStandardFootprint):
    ...
    def __init__(
        self, trip_current: RangeLike, *, hold_current: RangeLike = RangeExpr.ALL, voltage: RangeLike = RangeExpr.ZERO
    ) -> None:
    ...
    def contents(self) -> None:
        ...
        self.description = DescriptionString(
            "<b>trip current:</b> ",
            DescriptionString.FormatUnits(self.actual_trip_current, "A"),
            " <b>of spec:</b> ",
            DescriptionString.FormatUnits(self.trip_current, "A"),
            "\n",
            "<b>hold current:</b> ",
            DescriptionString.FormatUnits(self.actual_hold_current, "A"),
            " <b>of spec:</b> ",
            DescriptionString.FormatUnits(self.hold_current, "A"),
            "\n",
            "<b>voltage rating:</b> ",
        ...
        self.require(self.actual_trip_current.within(self.trip_current), "trip current not within specified rating")
        self.require(self.actual_hold_current.within(self.hold_current), "hold current not within specified rating")
        self.require(self.voltage.within(self.actual_voltage_rating), "operating voltage not within rating")
...
class TableFuse(PartsTableSelector, Fuse):
    ...
    def _row_filter(self, row: PartsTableRow) -> bool:
        return (
            ...
            and row[self.TRIP_CURRENT].fuzzy_in(self.get(self.trip_current))
            and row[self.HOLD_CURRENT].fuzzy_in(self.get(self.hold_current))
            and self.get(self.voltage).fuzzy_in(row[self.VOLTAGE_RATING])
        )
