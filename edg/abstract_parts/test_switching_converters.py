...
class BuckPowerPathTestTop(DesignTop):
    def __init__(self) -> None:
        ...
        self.dut = self.Block(
            BuckConverterPowerPath(
                input_voltage=Range(4, 6),
                output_voltage=(2, 3),
                frequency=Range.exact(100e3),
                output_current=Range(0.2, 1),
                sw_current_limits=Range(0, 2),
                input_voltage_ripple=75 * mVolt,
    ...
    def refinements(self) -> Refinements:
        return Refinements(
            ...
            instance_values=[(["dut", "inductor", "actual_inductance"], Range.from_tolerance(33e-6, 0.05))],
        )
    ...
    def __init__(self) -> None:
        ...
        self.dut = self.Block(
            BoostConverterPowerPath(
                input_voltage=Range(4, 6),
                output_voltage=(10, 14),
                frequency=Range.exact(200e3),
                output_current=Range(0.2, 0.5),
                sw_current_limits=Range(0, 2),
                input_voltage_ripple=75 * mVolt,
                output_voltage_ripple=25 * mVolt,
            )
