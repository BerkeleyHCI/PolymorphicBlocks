...
class PassiveConnector(DiscreteComponent, Block):
    ...
    def connected(
        self: PassiveConnectorSelfType, pins: Mapping[str, Union[CircuitPort, "HasPassivePort"]]
        ...
        for pin_name, pin_port in pins.items():
            cast(Block, builder.get_enclosing_block()).connect(pin_port, self.pins.request(pin_name))
        return self
...
class FootprintPassiveConnector(PassiveConnector, GeneratorBlock, FootprintBlock):
    ...
    def generate(self) -> None:
        ...
        self.require(
            max_pin_index <= self.actual_length, f"maximum pin index {max_pin_index} over requested length {length}"
        )
