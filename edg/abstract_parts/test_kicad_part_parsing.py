...
class KicadPartParsingTest(unittest.TestCase):
    ...
    def test_capacitor(self) -> None:
        self.assertEqual(
            Capacitor.parse_capacitor("0.1uF 6.3V"), (Range.from_tolerance(0.1e-6, 0.20), Range.zero_to_upper(6.3))
        )
        self.assertEqual(
            Capacitor.parse_capacitor("4.7u 6.3V"), (Range.from_tolerance(4.7e-6, 0.20), Range.zero_to_upper(6.3))
        )
        self.assertEqual(
            Capacitor.parse_capacitor("0.1uF 10% 12V"), (Range.from_tolerance(0.1e-6, 0.10), Range.zero_to_upper(12))
        )
