...
class ESeriesUtil:
    ...
    @classmethod
    def choose_preferred_number(cls, within: Range, series: Sequence[float], tolerance: float) -> Optional[float]:
        if within.lower == 0:
    ...
    @classmethod
    def series_of(
        cls, value: float, *, default: Optional[SeriesDefaultType] = None
    ) -> Union[None, int, SeriesDefaultType]:
...
class ESeriesRatioUtil(Generic[ESeriesRatioValueType]):
    ...
    def _no_result_error(
        self, best_values: Tuple[float, float], best: ESeriesRatioValueType, target: ESeriesRatioValueType
    ) -> Exception:
        ...
        return self.NoMatchException(f"No ratio found for target {target}, " f"best: {best}, with values {best_values}")
    ...
    def _generate_e_series_product(series: List[float], r1_decade: int, r2_decade: int) -> List[Tuple[float, float]]:
        ...
        zig-zag fashion."""
        r1_series = [ESeriesUtil.round_sig(elt * (10**r1_decade), ESeriesUtil.ROUND_DIGITS) for elt in series]
        r2_series = [ESeriesUtil.round_sig(elt * (10**r2_decade), ESeriesUtil.ROUND_DIGITS) for elt in series]
        out = []
    ...
    def find(self, target: ESeriesRatioValueType) -> Tuple[float, float]:
        ...
        while search_queue:
            ...
            product = self._generate_e_series_product(self.series, r1r2_decade[0], r1r2_decade[1])

            for r1, r2 in product:
                output = self.value_type.from_resistors(
                    Range.from_tolerance(r1, self.tolerance), Range.from_tolerance(r2, self.tolerance)
                )
    ...
    def _get_next_decades(self, decade: Tuple[int, int], target: ESeriesRatioValueType) -> List[Tuple[int, int]]:
        ...
        new_decades = [
            ...
            if self.value_type.from_resistors(range_of_decade(decade[0]), range_of_decade(decade[1])).intersects(target)
        ]
        return new_decades
