...
class DigitalIsolator(Interface, GeneratorBlock):
    ...
    def __init__(self) -> None:
        ...
        self.generator_param(
            self.in_a.requested(), self.out_b.requested(), self.in_b.requested(), self.out_a.requested()
        )

    @override
    def generate(self) -> None:  # validity checks
        super().generate()
        assert self.get(self.in_a.requested()) == self.get(
            self.out_b.requested()
        ), "in_a requested and out_b requested must be equal"
        assert self.get(self.in_b.requested()) == self.get(
