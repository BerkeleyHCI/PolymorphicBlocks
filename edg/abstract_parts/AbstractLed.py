...
class Led(DiscreteSemiconductor, HasStandardFootprint):
    _STANDARD_FOOTPRINT = lambda: LedStandardFootprint
...
class TableLed(PartsTableSelector, Led):
    ...
    def _row_filter(self, row: PartsTableRow) -> bool:
        return super()._row_filter(row) and (not self.get(self.color) or row[self.COLOR] == self.get(self.color))
...
class IndicatorLed(Light):
    ...
    def __init__(self, color: LedColorLike = Led.Any, *, current_draw: RangeLike = (1, 10) * mAmp) -> None:
        ...
        self.res = self.Block(
            Resistor(
                resistance=(
                    self.signal.link().voltage.upper() / self.target_current_draw.upper(),
...
class IndicatorLedArray(Light, GeneratorBlock):
    ...
    def __init__(self, count: IntLike, color: LedColorLike = Led.Any, *, current_draw: RangeLike = (1, 10) * mAmp):
        super().__init__()
...
class IndicatorSinkLedResistor(IndicatorSinkLed):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.res = self.Block(
            Resistor(
                resistance=(
                    self.signal.link().voltage.upper() / self.current_draw.upper(),
...
class VoltageIndicatorLed(Light):
    ...
    def __init__(self, color: LedColorLike = Led.Any, *, current_draw: RangeLike = (1, 10) * mAmp) -> None:
        ...
        self.signal = self.Port(VoltageSink(current_draw=RangeExpr()), [Power, InOut])
        self.gnd = self.Port(Ground(), [Common])
...
class IndicatorSinkRgbLed(Light):
    ...
    def __init__(self, current_draw: RangeLike = (1, 10) * mAmp) -> None:
        ...
        self.red_res = self.Block(
            Resistor(
                resistance=(
                    signal_red.link().voltage.upper() / self.target_current_draw.upper(),
                    signal_red.link().output_thresholds.upper() / self.target_current_draw.lower(),
                )
            )
        )
        self.green_res = self.Block(
            Resistor(
                resistance=(
                    signal_green.link().voltage.upper() / self.target_current_draw.upper(),
                    signal_green.link().output_thresholds.upper() / self.target_current_draw.lower(),
                )
            )
        )
        self.blue_res = self.Block(
            Resistor(
                resistance=(
                    signal_blue.link().voltage.upper() / self.target_current_draw.upper(),
