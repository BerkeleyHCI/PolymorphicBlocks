...
class DividerValues(ESeriesRatioValue["DividerValues"]):
    ...
    def from_resistors(r1_range: Range, r2_range: Range) -> "DividerValues":
        ...
        ratio = 1 / (R1 / R2 + 1)
        """
        return DividerValues(1 / (r1_range / r2_range + 1), 1 / (1 / r1_range + 1 / r2_range))
    ...
    def intersects(self, spec: "DividerValues") -> bool:
        return self.ratio.intersects(spec.ratio) and self.parallel_impedance.intersects(spec.parallel_impedance)
...
class ResistiveDivider(InternalSubcircuit, KiCadImportableBlock, GeneratorBlock):
    ...
    def divider_output(cls, vtop: RangeExpr, vbot: RangeExpr, ratio: RangeExpr) -> RangeExpr:
        ...
        return RangeExpr._to_expr_type(
            (
                (vtop.lower() - vbot.lower()) * ratio.lower() + vbot.lower(),
                (vtop.upper() - vbot.upper()) * ratio.upper() + vbot.upper(),
            )
        )

    def __init__(
        self, ratio: RangeLike, impedance: RangeLike, *, series: IntLike = 24, tolerance: FloatLike = 0.01
    ) -> None:
        super().__init__()
    ...
    def contents(self) -> None:
        ...
        self.description = DescriptionString(
            ...
            "\n<b>impedance:</b> ",
            DescriptionString.FormatUnits(self.actual_impedance, "Ω"),
            " <b>of spec:</b> ",
            DescriptionString.FormatUnits(self.impedance, "Ω"),
        )
    ...
    def generate(self) -> None:
        ...
        top_resistance, bottom_resistance = calculator.find(
            DividerValues(self.get(self.ratio), self.get(self.impedance))
        )

        self.top_res = self.Block(Resistor(resistance=Range.from_tolerance(top_resistance, self.get(self.tolerance))))
        self.bottom_res = self.Block(
            Resistor(resistance=Range.from_tolerance(bottom_resistance, self.get(self.tolerance)))
        )
        ...
        self.assign(
            self.actual_impedance, 1 / (1 / self.top_res.actual_resistance + 1 / self.bottom_res.actual_resistance)
        )
        self.assign(self.actual_series_impedance, self.top_res.actual_resistance + self.bottom_res.actual_resistance)
        self.assign(
            self.actual_ratio, self.divider_ratio(self.top_res.actual_resistance, self.bottom_res.actual_resistance)
        )
...
class BaseVoltageDivider(KiCadImportableBlock):
    ...
    def __init__(self, impedance: RangeLike) -> None:
        ...
        self.input = self.Port(VoltageSink(current_draw=RangeExpr(), voltage_limits=RangeExpr.ALL), [Input])
...
class FeedbackVoltageDivider(Analog, BaseVoltageDivider):
    ...
    def __init__(self, *, output_voltage: RangeLike, impedance: RangeLike, assumed_input_voltage: RangeLike) -> None:
        super().__init__(impedance=impedance)
