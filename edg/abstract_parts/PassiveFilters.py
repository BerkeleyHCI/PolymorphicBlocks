...
class LowPassRc(AnalogFilter, Block):
    ...
    def contents(self) -> None:
        ...
        self.c = self.Block(
            Capacitor(  # cutoff frequency is 1/(2 pi R C)
                capacitance=(1 / (2 * pi * self.cutoff_freq)).shrink_multiply(1 / self.impedance), voltage=self.voltage
            )
        )
...
class PullupDelayRc(DigitalFilter, Block):
    ...
    def connected(
        ...
        *,
        ...
        pwr: Optional[Port[VoltageLink]] = None,
        io: Optional[Port[DigitalLink]] = None,
    ) -> "PullupDelayRc":
...
class LowPassAnalogDifferentialRc(AnalogFilter, KiCadImportableBlock):
    ...
    def symbol_pinning(self, symbol_name: str) -> Mapping[str, BasePort]:
        ...
        return {"1": self.inp, "2": self.inn, "3": self.outp, "4": self.outn}
