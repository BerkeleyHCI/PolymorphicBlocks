...
class HighSideSwitch(PowerSwitch, KiCadSchematicBlock, GeneratorBlock):
    ...
    def __init__(
        ...
        *,
        clamp_voltage: RangeLike = RangeExpr.ZERO,
        clamp_resistance_ratio: FloatLike = 10,
    ) -> None:
        super().__init__()
    ...
    def generate(self) -> None:
        ...
        self.pull = self.Block(
            Resistor(resistance=pull_resistance, power=(0, pull_power_max), voltage=(0, pwr_voltage.upper()))
        )
        ...
        self.drv = self.Block(
            SwitchFet.PFet(
                ...
                frequency=self.frequency,
                drive_current=(
                    -1 * pwr_voltage.lower() / pull_resistance.upper(),
                    pwr_voltage.lower() / low_amp_rds_max,
                ),  # TODO simultaneously solve both FETs
            )
        )
        ...
        if clamp_voltage == Range(0, 0):  # no clamp
            self.import_kicad(
                self.file_path("resources", f"{self.__class__.__name__}.kicad_sch"), conversions=conversions
            )
        else:
            ...
            self.import_kicad(
                self.file_path("resources", f"{self.__class__.__name__}_Zener.kicad_sch"), conversions=conversions
            )
