...
class Resistor(PassiveComponent, KiCadInstantiableBlock, HasStandardFootprint):
    ...
    def __init__(
        self, resistance: RangeLike, power: RangeLike = RangeExpr.ZERO, voltage: RangeLike = RangeExpr.ZERO
    ) -> None:
        super().__init__()
    ...
    def contents(self) -> None:
        ...
        self.description = DescriptionString(
            ...
            " <b>of spec</b> ",
            DescriptionString.FormatUnits(self.resistance, "Ω"),
            "\n",
            "<b>power rating:</b> ",
...
class TableResistor(PartsTableSelector, Resistor):
    ...
    def _row_filter(self, row: PartsTableRow) -> bool:
        return (
            ...
            and row[self.RESISTANCE].fuzzy_in(self.get(self.resistance))
    ...
    def _row_sort_by(cls, row: PartsTableRow) -> Any:
        return (
            ESeriesUtil.series_of(row[cls.RESISTANCE].center(), default=ESeriesUtil.SERIES_MAX + 1),
...
class SeriesResistor(Resistor, GeneratorBlock):
    ...
    def generate(self) -> None:
        ...
        for i in range(count):
            self.res[i] = res = self.Block(Resistor(resistance=elt_resistance, power=elt_power, voltage=elt_voltage))
