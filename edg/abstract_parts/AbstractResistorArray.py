...
class ResistorArray(MultipackDevice, MultipackBlock, HasStandardFootprint):
    ...
    _STANDARD_FOOTPRINT = lambda: ResistorArrayStandardFootprint
    ...
    def contents(self) -> None:
        ...
        self.description = DescriptionString(  # TODO better support for array typed
            "<b>count:</b> ",
            DescriptionString.FormatUnits(self.actual_count, ""),  # TODO unitless
            " <b>of spec</b> ",
            DescriptionString.FormatUnits(self.count, ""),
            "\n",
            "<b>resistance:</b> ",
            DescriptionString.FormatUnits(self.actual_resistance, "Ω"),
            " <b>of specs</b> ",
            DescriptionString.FormatUnits(self.resistances, "Ω"),
            "\n",
            "<b>element power:</b> ",
            DescriptionString.FormatUnits(self.actual_power_rating, "W"),
            " <b>of operating:</b> ",
            DescriptionString.FormatUnits(self.powers, "W"),
        )
...
class TableResistorArray(PartsTableSelector, ResistorArray):
    ...
    def _row_filter(self, row: PartsTableRow) -> bool:
        ...
        return (
            ...
            and (self.get(self.count) == 0 or self.get(self.count) == row[self.COUNT])
            and (
                row[self.COUNT] >= len(self.get(self.a.requested()))
                and row[self.COUNT] >= len(self.get(self.b.requested()))
            )
            and row[self.RESISTANCE].fuzzy_in(resistance_intersect)
            and powers_hull.fuzzy_in(row[self.POWER_RATING])
        )
