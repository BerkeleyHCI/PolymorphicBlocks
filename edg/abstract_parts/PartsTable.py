...
class PartsTableRow:
    ...
    def __getitem__(self, item: Union[str, PartsTableColumn[PartsTableColumnType]]) -> Union[str, PartsTableColumnType]:
...
class PartsTable:
    ...
    def filter(self, fn: Callable[[PartsTableRow], bool]) -> PartsTable:
        ...
        new_rows = [row for row in self.rows if fn(row)]
        return PartsTable(new_rows)

    def map_new_columns(
        self, fn: Callable[[PartsTableRow], Optional[Dict[PartsTableColumn, Any]]], overwrite: bool = False
    ) -> PartsTable:
        ...
        for row in self.rows:
            ...
            if first_keys is None:
                ...
                assert (
                    first_keys.isdisjoint(row.values.keys()) or overwrite
...
class ExperimentalUserFnPartsTable(PartsTable):
    ...
    @staticmethod
    def user_fn(
        param_types: Sequence[Type[Any]] = [],
    ) -> Callable[[Callable[UserFnMetaParams, UserFnType]], UserFnSerialiable[UserFnMetaParams, UserFnType]]:
        def decorator(fn: Callable[UserFnMetaParams, UserFnType]) -> UserFnSerialiable[UserFnMetaParams, UserFnType]:
            ...
            if (
                fn.__name__ in ExperimentalUserFnPartsTable._user_fns
                or fn in ExperimentalUserFnPartsTable._fn_name_dict
            ):
    ...
    def serialize_fn(
        cls,
        fn: UserFnSerialiable[UserFnMetaParams, UserFnType],
        *args: UserFnMetaParams.args,
        **kwargs: UserFnMetaParams.kwargs,
    ) -> str:
