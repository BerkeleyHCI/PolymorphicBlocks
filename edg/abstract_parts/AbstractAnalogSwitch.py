...
class AnalogSwitch(Interface, KiCadImportableBlock, Block):
    ...
    def symbol_pinning(self, symbol_name: str) -> Dict[str, BasePort]:
        ...
        pins: Dict[str, BasePort] = {"C": self.com, "S": self.control, "V+": self.pwr, "V-": self.gnd}
...
class AnalogSwitchTree(AnalogSwitch, GeneratorBlock):
    ...
    def generate(self) -> None:
        ...
        while len(ports) > 1:  # stages in the tree
            ...
            stage_control_ios = [
                self.control.append_elt(DigitalSink.empty(), f"{switch_stage}_{i}") for i in range(stage_num_controls)
            ]
        ...
        self.assign(
            self.analog_current_limits, all_switches[0].analog_current_limits / (switch_size ** (switch_stage - 1))
        )
...
class AnalogMuxer(Interface, KiCadImportableBlock, GeneratorBlock):
    ...
    def symbol_pinning(self, symbol_name: str) -> Dict[str, BasePort]:
        assert symbol_name.startswith("edg_importable:Mux")  # can be any Mux
        count = int(symbol_name.removeprefix("edg_importable:Mux"))
        pins: Dict[str, BasePort] = {"C": self.out, "S": self.control, "V+": self.pwr, "V-": self.gnd}
    ...
    def mux_to(
        self, inputs: Optional[List[Port[AnalogLink]]] = None, output: Optional[Port[AnalogLink]] = None
    ) -> "AnalogMuxer":
