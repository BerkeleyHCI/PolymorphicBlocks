...
class PeripheralFixedPin(BaseLeafPinMapResource):
    ...
    def __eq__(self, other: Any) -> bool:
        # TODO avoid using is if we can compare port model equality
        return (
            isinstance(other, PeripheralFixedPin)
...
class PeripheralAnyResource(BaseDelegatingPinMapResource):
    ...
    def __eq__(self, other: Any) -> bool:
        # TODO avoid using is if we can compare port model equality
        return (
            isinstance(other, PeripheralAnyResource) and self.name == other.name and self.port_model is other.port_model
        )
...
class PeripheralFixedResource(BaseDelegatingPinMapResource):
    ...
    def __eq__(self, other: Any) -> bool:
        # TODO avoid using is if we can compare port model equality
        return (
            isinstance(other, PeripheralFixedResource)
            and self.name == other.name
            and self.port_model is other.port_model
            and self.inner_allowed_names == other.inner_allowed_names
        )
...
class AllocatedResource(NamedTuple):
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return (
            ...
            and self.name == other.name
            and self.resource_name is other.resource_name
...
class UserAssignmentDict:
    ...
    @staticmethod
    def _from_list(
        assignment_spec_list: List[Tuple[List[str], str]], name: List[str]
...
class PinMapUtil:
    ...
    def allocate(
        self, port_types_names: List[Tuple[Type[Port], List[str]]], assignments_spec: List[str] = []
        ...
        def try_allocate_resource(
            port_type: Type[Port], port_name: str, resource: BasePinMapResource, sub_assignments: UserAssignmentDict
        ) -> AllocatedResource:
            ...
            if isinstance(resource, PinResource):  # single pin: just assign it
                ...
                for inner_name, inner_model in resource.port_model._ports.items():
                    ...
                    if isinstance(
                        resource, PeripheralFixedResource
                    ):  # filter further by allowed pins for this peripheral
                        allowed_names = resource.inner_allowed_names.get(inner_name, [])
                        allowed_resources = list(
                            itertools.chain(*[resources_by_name.get(name, []) for name in allowed_names])
                        )
                        resource_pool = [resource for resource in resource_pool if resource in allowed_resources]

                    inner_allocation = allocate_port_type(
                        resource_pool, inner_type, f"{port_name}.{inner_name}", inner_assignment, inner_sub_assignments
                    )
                    ...
                    if (
                        type(inner_model) in self.transforms
        ...
        def allocate_port_type(
            resource_pool: List[BasePinMapResource],
            port_type: Type[Port],
            port_name: str,
            assignment: Optional[str],
            sub_assignments: UserAssignmentDict,
        ) -> AllocatedResource:
