...
from ..jlc.JlcFerriteBead import JlcFerriteBead
...
class JlcPartsFerriteBead(PartsTableSelectorFootprint, JlcPartsBase, TableFerriteBead):
    ...
    def _entry_to_table_row(
        ...
        try:
            ...
            try:  # sometimes '-'
                current_rating = PartParserUtil.parse_value(attributes.get("Current rating", str), "A")
            ...
            if row_dict[cls.PART_NUMBER_COL] == "GZ2012D101TF":  # additional data for a basic part
                row_dict[cls.DC_RESISTANCE] = Range.exact(
                    0.15
                )  # from https://evelta.com/content/datasheets/212-Ferrite-2012.pdf
            else:
                # Dc resistance sometimes NaN
                row_dict[cls.DC_RESISTANCE] = Range.exact(attributes.get("Dc resistance", float, sub="resistance"))
            row_dict[cls.HF_IMPEDANCE] = Range.exact(attributes.get("Impedance @ frequency", float, sub="esr"))
