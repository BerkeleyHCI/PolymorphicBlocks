...
class JlcPartsBjt(PartsTableSelectorFootprint, JlcPartsBase, TableBjt):
    ...
    def _entry_to_table_row(
        ...
        try:
            ...
            row_dict[cls.VCE_RATING] = Range.zero_to_upper(
                PartParserUtil.parse_value(attributes.get("Collector-emitter breakdown voltage (vceo)", str), "V")
            )
            row_dict[cls.ICE_RATING] = Range.zero_to_upper(
                PartParserUtil.parse_value(attributes.get("Collector current (ic)", str), "A")
            )
            row_dict[cls.GAIN] = Range.exact(
                PartParserUtil.parse_value(attributes.get("Dc current gain (hfe@ic,vce)", str).split("@")[0], "")
            )
