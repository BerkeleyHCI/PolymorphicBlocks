...
class JlcPartsBaseFet(JlcPartsBase, BaseTableFet):
    ...
    def _entry_to_table_row(
        ...
        try:
            ...
            row_dict[cls.IDS_RATING] = Range.zero_to_upper(
                PartParserUtil.parse_value(attributes.get("Continuous drain current (id)", str), "A")
            )
            ...
            row_dict[cls.VGS_DRIVE] = Range(
                attributes.get("Gate threshold voltage (vgs(th)@id)", float, sub="Vgs"), vgs_for_ids
            )
            ...
            try:
                input_capacitance: Optional[float] = attributes.get(
                    "Input capacitance (ciss@vds)", float, sub="capacity"
                )
