...
def valuelit_to_lit(expr: ValueLit) -> LitTypes:
    ...
    elif expr.HasField("range") and expr.range.minimum.HasField("floating") and expr.range.maximum.HasField("floating"):
...
def localpath_concat(
    *elts: Union[LocalPath, str, Allocate, "Reserved.V"]
) -> LocalPath:  # TODO workaround for broken enum typing
    result = LocalPath()
    for elt in elts:
...
def local_path_to_str_list(path: LocalPath) -> List[str]:
    ...
    def step_to_str(step: LocalStep) -> str:
        if step.HasField("name"):
            ...
            return {CONNECTED_LINK: "(link)", IS_CONNECTED: "(is_connected)"}[step.reserved_param]
        else:
...
def add_pair(
    ...
    name: str,
) -> Union[PortLike, BlockLike, LinkLike, ValInit, ValueExpr]:
    elt = pb.add()
    elt.name = name
    return elt.value
...
def pair_get_opt(
    ...
    name: str,
) -> Union[Optional[PortLike], Optional[BlockLike], Optional[LinkLike], Optional[ValInit], Optional[ValueExpr]]:
    for elt in pb:
        if elt.name == name:
            return elt.value
    return None
