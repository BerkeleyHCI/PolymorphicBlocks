...
class Path(NamedTuple):  # internal helper type
    ...
    def startswith(self, prefix: Path) -> bool:
        if self.blocks == prefix.blocks:  # exact match, check subpaths
            ...
            return (
                (not self.links)
                and (not prefix.links)
                and (not self.ports)
                and (not prefix.ports)
                and (not self.params)
                and (not prefix.params)
            )
        else:  # no match
            return False
    ...
    def _follow_partial_steps(
        self, steps: List[edgir.LocalStep], curr: edgir.EltTypes
    ...
    def follow_partial(
        self, path: edgir.LocalPath, curr: edgir.EltTypes
    ) -> Tuple[Optional[List[edgir.LocalStep]], Tuple[Path, edgir.EltTypes]]:
        ...
        try:
            ...
            raise type(e)(f"(while following {edgir.local_path_to_str(path)} from {self}) " + str(e)).with_traceback(
                sys.exc_info()[2]
            )
    ...
    def follow(self, path: edgir.LocalPath, curr: edgir.EltTypes) -> Tuple[Path, edgir.EltTypes]:
        ...
        if remaining is not None:
            raise ValueError(
                f"can't follow {edgir.local_path_to_str(path)} from {self}, have unused path elements {remaining} from {end_path}"
            )
        else:
...
class Transform:
    ...
    def _traverse_portlike(self, context: TransformContext, elt: edgir.PortLike) -> None:
        try:
            ...
            raise type(e)(f"(while visiting PortLike at {context}) " + str(e)).with_traceback(sys.exc_info()[2])
    ...
    def _traverse_block(self, context: TransformContext, elt: edgir.HierarchyBlock) -> None:
        try:
            ...
            raise type(e)(f"(while visiting Block at {context}) " + str(e)).with_traceback(sys.exc_info()[2])
    ...
    def _traverse_linklike(self, context: TransformContext, elt: edgir.LinkLike) -> None:
        try:
            ...
            raise type(e)(f"(while visiting LinkLike at {context}) " + str(e)).with_traceback(sys.exc_info()[2])
        ...
        if elt.HasField("lib_elem"):
            ...
            try:
                ...
                raise type(e)(f"(while visiting Link at {context}) " + str(e)).with_traceback(sys.exc_info()[2])
            ...
            try:
                ...
                raise type(e)(f"(while visiting LinkArray at {context}) " + str(e)).with_traceback(sys.exc_info()[2])
