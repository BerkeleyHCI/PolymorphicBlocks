...
class Connection:
    ...
    class ConnectedLink(NamedTuple):  # link-mediated connection (including bridged ports and inner links)
        link_type: Type[Link]
        is_link_array: bool
        bridged_connects: List[
            Tuple[BasePort, edgir.LocalPath]
        ]  # external / boundary port <> link port, invalid in links
    ...
    def __init__(self, parent: BaseBlock, flatten: bool) -> None:
        ...
        self.link_connected_ports: IdentityDict[BasePort, List[BasePort]] = (
            IdentityDict()
        )  # link port -> [connected ports]
        self.bridged_ports: IdentityDict[BasePort, BasePort] = (
            IdentityDict()
        )  # external port -> internal port on bridge
    ...
    def add_ports(self, ports: Iterable[BasePort]) -> None:
            ...
            if is_export:
                ...
                if initializers:
                    raise UnconnectableError(
                        f"Connected boundary port {ext_port._name_from(self.parent, allow_unknown=True)} may not have initializers, "
        ...
        for port in ports_to_connect:
            if isinstance(self.parent, Block):  # check if bridge is needed
                if port._block_parent() is self.parent:
                    if port._get_initializers([]):
                        raise UnconnectableError(
                            f"Connected boundary port {port._name_from(self.parent, allow_unknown=True)} may not have initializers"
                        )
                    if not isinstance(port, Port):
                        raise UnconnectableError(
                            f"Can't generate bridge for non-Port {port._name_from(self.parent, allow_unknown=True)}"
                        )
                ...
                if port._block_parent() is not self.parent:
                    raise UnconnectableError(
                        f"Port {port._name_from(self.parent, allow_unknown=True)} not in containing link"
                    )
                link_facing_port = port
            else:
            ...
            if self._baseport_leaf_type(link_facing_port).link_type is not type(link):
                raise UnconnectableError(
                    f"Can't connect {port._name_from(self.parent, allow_unknown=True)} to link of type {type(link)}"
                )
            ...
            if allocatable_link_ports is None:
                raise UnconnectableError(
                    f"No link port for {port._name_from(self.parent, allow_unknown=True)} of type {port_type}"
                )
            if not allocatable_link_ports:
                raise UnconnectableError(
                    f"No remaining link ports to {port._name_from(self.parent, allow_unknown=True)}"
                )
...
class BaseBlock(HasMetadata, metaclass=BaseBlockMeta):
    ...
    def __init__(self) -> None:
        ...
        self.description: Optional[DescriptionString] = (
            None  # additional string field to be displayed as part of the tooltip for blocks
        )
        ...
        for name, port in self._ports.items():
            for param, path, initializer in port._get_initializers([name]):
                AssignBinding.populate_assign_proto(
                    edgir.add_pair(pb.constraints, f"(init){'.'.join(path)}"),
                    param,
                    param._to_expr_type(initializer),
                    ref_map,
                )
        ...
        def check_subexpr(expr: Union[ConstraintExpr, BasePort]) -> None:  # TODO rewrite this whole method
            if isinstance(expr, ConstraintExpr) and isinstance(expr.binding, ParamBinding):
                ...
                if not (expr_parent is self or expr_parent_parent is self):
                    raise UnreachableParameterError(
                        f"In {self}, constraint references unreachable parameter {expr}. "
                ...
                if not block_parent is self or block_parent._parent is self:
                    raise UnreachableParameterError(
                        f"In {self}, constraint references unreachable port {expr}. "
    ...
    def assign(
        self,
        target: ConstraintExpr[ConstrType, Any],
    ...
    def Port(self, tpe: T, *, optional: bool = False, doc: Optional[str] = None) -> T:
        ...
        if self._elaboration_state != BlockElaborationState.init:
            raise BlockDefinitionError(
                type(self),
                "can't call Port(...) outside __init__",
    ...
    def Parameter(self, tpe: ConstraintType, *, doc: Optional[str] = None) -> ConstraintType:
        ...
        if self._elaboration_state != BlockElaborationState.init:
            raise BlockDefinitionError(
                type(self),
                "can't call Parameter(...) outside __init__",
    ...
    def connect(self, *connects: Union[BasePort, Connection], flatten: bool = False) -> Connection:
        ...
        for port in connects_ports_new:
            if port._block_parent() is not self:
                ...
                if not (
                    port._parent is enclosing_block
                    or (isinstance(port._parent, Vector) and port._parent._parent is enclosing_block)
                ):
