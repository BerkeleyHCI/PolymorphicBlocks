...
from .MultipackBlock import (
    MultipackBlock,
    PackedBlockAllocate,
    PackedBlockPortArray,
    PackedBlockParamArray,
    PackedBlockParam,
)
...
class DesignTop(Block):
    ...
    def __init__(self) -> None:
        ...
        self._packed_blocks = IdentityDict[
            Union[Block, PackedBlockAllocate], DesignPath
        ]()  # multipack part -> packed block (as path)
    ...
    def refinements(self) -> Refinements:
        ...
        def make_packing_refinement(
            multipack_part: Union[Block, PackedBlockAllocate], path: DesignPath
        ...
        return Refinements(
            instance_refinements=[
                make_packing_refinement(multipack_part, path) for multipack_part, path in self._packed_blocks.items()
            ]
        )
    ...
    def _populate_def_proto_block_contents(self, pb: edgir.BlockLikeTypes, ref_map: Refable.RefMapType) -> None:
        ...
        for multipack_part, packed_path in self._packed_blocks.items():
            ...
            for exterior_port, packed_port in packing_rule.tunnel_exports.items():
                ...
                exported_tunnel = edgir.add_pair(
                    pb.constraints, f"(packed){multipack_name}.{part_name}.{packed_port_name}"
                ).exportedTunnel
            ...
            for multipack_param, packed_param in packing_rule.tunnel_assigns.items():
                if isinstance(packed_param, ConstraintExpr):
                    ...
                    assign_tunnel = edgir.add_pair(
                        pb.constraints, f"(packed){multipack_name}.{part_name}.{packed_param_name}"
                    ).assignTunnel
            ...
            for multipack_param, unpacked_param in packing_rule.tunnel_unpack_assigns.items():
                if isinstance(unpacked_param, ConstraintExpr):
                    ...
                    assign_tunnel = edgir.add_pair(
                        pb.constraints, f"(unpacked){multipack_name}.{part_name}.{multipack_param_name}"
                    ).assignTunnel
