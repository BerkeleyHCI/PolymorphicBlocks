...
class BlockPrototype(Generic[BlockPrototypeType]):
    ...
    def __getattribute__(self, item: str) -> Any:
        if item.startswith("_"):
            ...
            raise AttributeError(
                f"{self.__class__.__name__} has no attributes, must bind to get a concrete instance, tried to get {item}"
            )
    ...
    def __setattr__(self, key: str, value: Any) -> None:
        if key.startswith("_"):
            ...
            raise AttributeError(
                f"{self.__class__.__name__} has no attributes, must bind to get a concrete instance, tried to set {key}"
            )
            ...
            for arg_name, arg_param in list(inspect.signature(orig_init).parameters.items())[1:]:
                ...
                if param_expr_type is None:
                    raise BlockDefinitionError(
                        new_cls,
                        f"in {new_cls}.__init__, unknown annotation type for {arg_name}: {arg_param.annotation}",
                    )
            ...
            def wrapped_init(self: Any, *args: Any, **kwargs: Any) -> None:
                ...
                try:
                    ...
                    for arg_pos, (arg_name, arg_param, param_expr_type) in enumerate(arg_data):
                        if arg_pos < len(args) and arg_param.kind in (
                            inspect.Parameter.POSITIONAL_ONLY,
                            inspect.Parameter.POSITIONAL_OR_KEYWORD,
                        ):  # present positional arg
                            new_arg = remap_arg(arg_name, param_expr_type, args[arg_pos])
                            new_args.append(new_arg)
                            self._init_params[arg_name] = new_arg
                        elif arg_pos >= len(args) and arg_param.kind in (
                            inspect.Parameter.POSITIONAL_ONLY,
                            ...
                            if len(builder.stack) == 1:  # at top-level, fill in all args
                                ...
                                new_args.append(new_arg)
                                self._init_params[arg_name] = new_arg
                        elif arg_name in kwargs and arg_param.kind in (
                            inspect.Parameter.POSITIONAL_OR_KEYWORD,
                            ...
                            self._init_params[arg_name] = new_arg
                        elif arg_name not in kwargs and arg_param.kind in (
                            inspect.Parameter.POSITIONAL_OR_KEYWORD,
                            inspect.Parameter.KEYWORD_ONLY,
                        ):  # non-present kwarg
                            if (
                                arg_param.default is not inspect._empty
                            ):  # default values do show up in kwargs, add them to transform them
...
class Block(BaseBlock, metaclass=BlockMeta):
    ...
    @override
    def _build_ref_map(
        self, ref_map: Refable.RefMapType, prefix: edgir.LocalPath, *, interface_only: bool = False
    ) -> None:
        ...
        for name, connect in self._connects.items_ordered():
            ...
            if connect_elts is None:  # single port net - effectively discard
                ...
                for idx, (self_port, link_port_path) in enumerate(connect_elts.bridged_connects):
                    ...
                    edgir.add_pair(pb.blocks, f"(bridge){port_name}").lib_elem.base.target.name = (
                        self_port.bridge_type._static_def_name()
                    )
                    ...
                    bridge_constraint_pb.exported.internal_block_port.ref.CopyFrom(
                        edgir.localpath_concat(bridge_path, "outer_port")
                    )

                    connect_constraint_pb = edgir.add_pair(pb.constraints, f"(conn){name}_b{idx}")
                    connect_constraint_pb.connected.block_port.ref.CopyFrom(
                        edgir.localpath_concat(bridge_path, "inner_link")
                    )
                    connect_constraint_pb.connected.link_port.ref.CopyFrom(
                ...
                for idx, (subelt_port, link_port_path) in enumerate(connect_elts.link_connects):
                    ...
                    if connect_elts.is_link_array:
                        ...
                        connect_constraint_pb.connectedArray.link_port.ref.CopyFrom(
                            edgir.localpath_concat(link_path, link_port_path)
                        )
                    else:
        ...
        for block_name, block in self._blocks.items():
            ...
            for block_param_name, block_param in all_block_params.items():
                if isinstance(block_param.binding, InitParamBinding) and block_param.binding.value is not None:
                    ...
                    AssignBinding.populate_assign_proto(
                        edgir.add_pair(pb.constraints, f"(init){block_name}.{block_param_name}"),
                        block_param,
                        param_typed_value,
                        ref_map,
                    )
    ...
    def chain(self, *elts: Union[Connection, BasePort, Block]) -> Any:
        ...
        if isinstance(elts[0], (BasePort, Connection)):
            ...
            if len(outable_ports) > 1:
                raise BlockDefinitionError(
                    type(self),
                    f"first element 0 to chain {type(elts[0])} does not have exactly one InOut or Output port: {outable_ports}",
                )
        ...
        for i, elt in list(enumerate(elts))[1:-1]:
            ...
            if elt._get_ports_by_tag({Input}) and elt._get_ports_by_tag({Output}):
                ...
                if len(in_ports) != 1:
                    raise ChainError(
                        type(self), f"element {i} to chain {type(elt)} does not have exactly one Input port: {in_ports}"
                    )
                elif len(out_ports) != 1:
                    raise ChainError(
                        type(self),
                        f"element {i} to chain {type(elt)} does not have exactly one Output port: {out_ports}",
                    )
                ...
                if len(ports) != 1:
                    raise ChainError(
                        type(self), f"element {i} to chain {type(elt)} does not have exactly one InOut port: {ports}"
                    )
                self.connect(current_port, ports[0])
                chain_blocks.append(elt)
            else:
                raise ChainError(
                    type(self), f"element {i} to chain {type(elt)} has no Input and Output, or InOut ports"
                )
        ...
        if isinstance(elts[-1], (BasePort, Connection)):
            ...
            if len(inable_ports) != 1:
                raise BlockDefinitionError(
                    type(self),
                    f"last element {len(elts) - 1} to chain {type(elts[-1])} does not have exactly one InOut or Input port: {inable_ports}",
                )
    ...
    def Export(
        ...
        *,
        ...
        if isinstance(port, BaseVector):  # TODO can the vector and non-vector paths be unified?
            ...
            new_port: BasePort = self.Port(Vector(port._tpe.empty()), tags, optional=optional, doc=doc)
        elif isinstance(port, Port):
            new_port = self.Port(
                type(port).empty(), tags, optional=optional, doc=doc  # TODO is dropping args safe in all cases?
            )
        else:
