...
class PortBridge(InternalBlock, Block):
    ...
    def Port(self, tpe: T, *args: Any, **kwargs: Any) -> T:
        assert (
            "optional" not in kwargs
        ), f"Ports in PortBridge are optional by default, required should be set by enclosing block, in {kwargs}"
...
class PortAdapter(InternalBlock, Block, Generic[AdapterDstType]):
    ...
    def Port(self, tpe: T, *args: Any, **kwargs: Any) -> T:
        assert (
            "optional" not in kwargs
        ), "Ports in PortBridge are optional by default, required should be set by enclosing block"
        return super().Port(tpe, *args, optional=True, **kwargs)
