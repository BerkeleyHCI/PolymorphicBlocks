...
class EltDictBlockProtoTestCase(unittest.TestCase):
  def test_connectivity(self):
    ...
    self.assertEqual(pb.blocks[0].value.lib_elem.base.target.name, "edg_core.test_common.TestBlockSink")
    self.assertEqual(pb.blocks[1].name, 'sink[1]')
    self.assertEqual(pb.blocks[1].value.lib_elem.base.target.name, "edg_core.test_common.TestBlockSink")
    self.assertEqual(pb.blocks[2].name, 'nested[inner][a]')
    self.assertEqual(pb.blocks[2].value.lib_elem.base.target.name, "edg_core.test_common.TestBlockSink")
    self.assertEqual(pb.blocks[3].name, 'nested[inner][b]')
    self.assertEqual(pb.blocks[3].value.lib_elem.base.target.name, "edg_core.test_common.TestBlockSink")
