...
class Vector(BaseVector, Generic[VectorType]):
    ...
    def _get_initializers(self, path_prefix: List[str]) -> List[Tuple[ConstraintExpr, List[str], ConstraintExpr]]:
        if self._elts is not None:
            return list(
                itertools.chain(*[elt._get_initializers(path_prefix + [name]) for (name, elt) in self._elts.items()])
            )
        else:
            return []
    ...
    def map_extract(
        self, selector: Callable[[VectorType], Union[ConstraintExpr, ExtractPortType]]
    ) -> Union[ArrayExpr, DerivedVector[ExtractPortType]]:
