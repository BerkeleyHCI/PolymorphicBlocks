...
class PortBridgeHierarchyBlockTop(Block):
    def __init__(self) -> None:
        ...
        self.source = self.Block(
            TestBlockSource()
        )  # TODO the PortBridge requires a connect, which isn't satisfied here
...
class DesignInstantiationTestCase(unittest.TestCase):
    def test_single_hierarchy(self) -> None:
        ...
        self.assertEqual(pb.blocks[0].value.hierarchy.self_class.target.name, "edg.core.test_common.TestBlockSource")
        self.assertEqual(pb.blocks[0].value.hierarchy.ports[0].name, "source")
        self.assertEqual(
            pb.blocks[0].value.hierarchy.ports[0].value.port.self_class.target.name,
            "edg.core.test_common.TestPortSource",
        )

        self.assertEqual(pb.blocks[1].name, "sink1")
        self.assertEqual(pb.blocks[1].value.hierarchy.self_class.target.name, "edg.core.test_common.TestBlockSink")
        self.assertEqual(pb.blocks[1].value.hierarchy.ports[0].name, "sink")
        self.assertEqual(
            pb.blocks[1].value.hierarchy.ports[0].value.port.self_class.target.name, "edg.core.test_common.TestPortSink"
        )

        self.assertEqual(pb.blocks[2].name, "sink2")
        self.assertEqual(pb.blocks[2].value.hierarchy.self_class.target.name, "edg.core.test_common.TestBlockSink")
        self.assertEqual(pb.blocks[2].value.hierarchy.ports[0].name, "sink")
        self.assertEqual(
            pb.blocks[2].value.hierarchy.ports[0].value.port.self_class.target.name, "edg.core.test_common.TestPortSink"
        )
        ...
        self.assertEqual(
            pb.links[0].value.link.ports[0].value.port.self_class.target.name, "edg.core.test_common.TestPortSource"
        )
        ...
        self.assertEqual(
            pb.links[0].value.link.ports[1].value.array.ports.ports[0].value.port.self_class.target.name,
            "edg.core.test_common.TestPortSink",
        )
        self.assertEqual(pb.links[0].value.link.ports[1].value.array.ports.ports[1].name, "1")
        self.assertEqual(
            pb.links[0].value.link.ports[1].value.array.ports.ports[1].value.port.self_class.target.name,
            "edg.core.test_common.TestPortSink",
        )
    ...
    def test_exported_hierarchy(self) -> None:
        ...
        self.assertEqual(pb.ports[0].value.port.self_class.target.name, "edg.core.test_common.TestPortSink")

        self.assertEqual(len(pb.blocks), 1)
        self.assertEqual(pb.blocks[0].name, "sink")
        self.assertEqual(pb.blocks[0].value.hierarchy.self_class.target.name, "edg.core.test_common.TestBlockSink")
        self.assertEqual(pb.blocks[0].value.hierarchy.ports[0].name, "sink")
        self.assertEqual(
            pb.blocks[0].value.hierarchy.ports[0].value.port.self_class.target.name, "edg.core.test_common.TestPortSink"
        )
    ...
    def test_bridge_hierarchy(self) -> None:
        ...
        self.assertEqual(pb.blocks[2].value.hierarchy.self_class.target.name, "edg.core.test_common.TestPortBridge")
        ...
        self.assertEqual(
            pb.blocks[2].value.hierarchy.ports[1].value.port.self_class.target.name,
            "edg.core.test_common.TestPortSource",
        )
