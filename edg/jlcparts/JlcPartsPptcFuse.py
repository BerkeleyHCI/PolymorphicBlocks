...
class JlcPartsPptcFuse(PptcFuse, TableFuse, SmdStandardPackageSelector, JlcPartsBase):
    ...
    def _entry_to_table_row(cls, row_dict: Dict[PartsTableColumn, Any], package: str, attributes: JlcPartsAttributes) \
        ...
        try:
            ...
            row_dict[cls.VOLTAGE_RATING] = Range.zero_to_upper(float(PartParserUtil.parse_value(
                attributes.get("Operating voltage (max)", str), 'V')))
            row_dict[cls.TRIP_CURRENT] = Range.exact(float(PartParserUtil.parse_value(
                attributes.get("Trip current", str), 'A')))
            row_dict[cls.HOLD_CURRENT] = Range.exact(float(PartParserUtil.parse_value(
                attributes.get("Hold current", str), 'A')))
