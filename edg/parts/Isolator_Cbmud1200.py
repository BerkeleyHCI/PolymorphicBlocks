...
class Cbmud1200l_Device(InternalSubcircuit, JlcPart, FootprintBlock):
    def __init__(self) -> None:
        ...
        in_model = DigitalSink.from_supply(
            self.gnd1, self.vdd1, voltage_limit_tolerance=(-0.3, 0.5), input_threshold_factor=(0.3, 0.7)
        )
        ...
        self.vdd2 = self.Port(
            VoltageSink.from_gnd(  # assumed the same as vdd1 ratings
                self.gnd2,
                voltage_limits=(2.5, 5.5) * Volt,
                current_draw=(280, 300) * uAmp + 2 * ((0, 26) * mAmp),  # 2.5-5v static to maximum dynamic power
            )
        )
        out_model = DigitalSource.from_supply(self.gnd2, self.vdd2, current_limits=(-50, 50) * mAmp)
...
class Cbmud1200l(DigitalIsolator, GeneratorBlock):
    ...
    def generate(self) -> None:
        super().generate()
        assert not self.get(self.in_b.requested()) and not self.get(
            self.out_a.requested()
        ), f"device has no b->a channels"
