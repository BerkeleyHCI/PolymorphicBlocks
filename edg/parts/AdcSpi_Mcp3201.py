...
class Mcp3201_Device(InternalSubcircuit, FootprintBlock):
    def __init__(self) -> None:
        super().__init__()
        self.vdd = self.Port(
            VoltageSink(voltage_limits=(2.7, 5.5) * Volt, current_draw=(0.5, 400) * uAmp)
        )  # from standby to operating
        self.vss = self.Port(Ground())

        self.vref = self.Port(
            VoltageSink(voltage_limits=(0.25 * Volt, self.vdd.link().voltage.upper()), current_draw=(0.001, 150) * uAmp)
        )
        self.inp = self.Port(
            AnalogSink.from_supply(
                self.vss,
                self.vref,
                voltage_limit_tolerance=(-0.6, 0.6) * Volt,
                signal_limit_tolerance=(-0.1, 0.1)
                * Volt,  # kinda, actually from IN- to Vref+IN-, for IN- within 0.1V of Vss
