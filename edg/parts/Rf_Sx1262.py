...
class Pe4259_Device(InternalSubcircuit, FootprintBlock, JlcPart):
    def __init__(self) -> None:
        ...
        self.require(
            self.restricted_availiability == False,
            "part has restricted availiability, DO NOT USE, see class def for details",
        )
...
class Sx1262_Device(InternalSubcircuit, FootprintBlock, JlcPart):
    def __init__(self) -> None:
        ...
        self.vbat = self.Port(
            VoltageSink.from_gnd(
                self.gnd, voltage_limits=(1.8, 3.7) * Volt, current_draw=(0.000160, 118) * mAmp  # include Vbat
            )
        )  # from IDDOFF to max TX
        self.vbat_io = self.Port(
            VoltageSink.from_gnd(self.gnd, voltage_limits=(1.8, 3.7) * Volt)
...
class Sx1262(Resettable, DiscreteRfWarning, Block):
    ...
    def contents(self) -> None:
        ...
        with self.implicit_connect(ImplicitConnect(self.gnd, [Common])) as imp:
            ...
            self.dcc_l = self.Block(
                Inductor(  # from datasheet 5.1.5
                    15 * uHenry(tol=0.2),
                    current=(0, 100) * mAmp,
                    frequency=20 * MHertz(tol=0),
                    resistance_dc=(0, 2) * Ohm,
                )
            )
            ...
            (self.tx_l, self.tx_pi), _ = self.chain(
                self.ic.rfo,
                imp.Block(
                    LLowPassFilterWith2HNotch(
                        915 * MHertz, 11.7 * Ohm, -4.8 * Ohm, 50 * Ohm, 0.2, rf_voltage, rf_current
                    )
                ),
                imp.Block(
                    PiLowPassFilter(
                        (915 - 915 / 2, 915 + 915 / 2) * MHertz,
                        50 * Ohm,
                        0,
                        50 * Ohm,
                        0.2,  # Q=1
            ...
            self.balun = imp.Block(
                Sx1262BalunLike(915 * MHertz, 74 * Ohm, -134 * Ohm, 50 * Ohm, 0.15, rf_voltage, rf_current)
            )
            ...
            (self.ant_pi, self.ant), _ = self.chain(
                ...
                imp.Block(
                    Antenna(
                        frequency=(915 - 0.5, 915 + 0.5) * MHertz,  # up to 500kHz bandwidth in LoRa mode
                        impedance=50 * Ohm(tol=0.1),
                        power=(0, 0.159) * Watt,
                    )
                ),
            )  # +22dBm
