...
class Cp2102_Device(InternalSubcircuit, FootprintBlock, JlcPart):
    def __init__(self) -> None:
        ...
        self.require(
            self.uart.is_connected()
            | self.ri.is_connected()
            | self.dcd.is_connected()
            | self.dtr.is_connected()
            | self.dsr.is_connected()
            | self.rts.is_connected()
            | self.cts.is_connected()
        )
