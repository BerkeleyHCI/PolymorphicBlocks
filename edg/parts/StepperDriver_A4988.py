...
class A4988_Device(InternalSubcircuit, FootprintBlock, JlcPart):
  def __init__(self) -> None:
    ...
    self.vdd = self.Port(VoltageSink(
      voltage_limits=(3, 5.5)*Volt, current_draw=RangeExpr())
    )
    self.vbb = self.Port(  # merges Vbb1/2, split rail not supported
      VoltageSink(voltage_limits=(8, 35)*Volt, current_draw=RangeExpr())
    )
  ...
  def contents(self) -> None:
    ...
    self.footprint(
      'U', 'Package_DFN_QFN:QFN-28-1EP_5x5mm_P0.5mm_EP3.35x3.35mm_ThermalVias',
      {
        # '1': self.nsleep,
        # '2': self.aout1,
        # '3': self.gnd,  # AISEN
