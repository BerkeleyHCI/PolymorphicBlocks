...
class A4988_Device(InternalSubcircuit, FootprintBlock, JlcPart):
    def __init__(self) -> None:
        ...
        vbb_model = VoltageSink(
            voltage_limits=(8, 35) * Volt, current_draw=RangeExpr()  # in operation; down to 0 during sleep
        )
        ...
        dout1_model = DigitalSource.from_supply(self.gnd, self.vbb1, current_limits=(-2, 2) * Amp)
...
class A4988(GeneratorBlock):
    ...
    def __init__(
        self,
        step_resolution: IntLike = 16,
        itrip: RangeLike = 1 * Amp(tol=0.15),
        itrip_vref: RangeLike = 0.25 * Volt(tol=0.08),
    ) -> None:
        super().__init__()
        ...
        self.itrip_vref = self.ArgParameter(
            itrip_vref, doc="voltage reference for Isense trip, not counting the 8x on Vref"
        )
    ...
    def contents(self) -> None:
        ...
        self.vcpi_cap = self.Block(
            Capacitor(0.1 * uFarad(tol=0.2), (0, 16) * Volt)
        ...
        for i, sensen in [("1", self.ic.sense1), ("2", self.ic.sense2)]:
            isense = self.isense[i] = self.Block(
                Resistor(self.itrip_vref / self.itrip, power=self.itrip_vref * self.itrip)  # TODO shrink tolerances
            )
