...
class Ld1117_Device(InternalSubcircuit, LinearRegulatorDevice, GeneratorBlock, JlcPart, FootprintBlock):
    ...
    def generate(self) -> None:
        ...
        suitable_parts = [
            (part_out, part_number, lcsc_part)
            for part_out, part_number, lcsc_part in parts
...
class Ap2204k(VoltageRegulatorEnableWrapper, LinearRegulator):
    ...
    def contents(self) -> None:
        ...
        self.in_cap = self.Block(DecouplingCapacitor(capacitance=1.1 * uFarad(tol=0.2))).connected(
            self.gnd, self.pwr_in
        )
        self.out_cap = self.Block(DecouplingCapacitor(capacitance=2.2 * uFarad(tol=0.2))).connected(
            self.gnd, self.pwr_out
        )
...
class Ap7215(LinearRegulator):
    ...
    def contents(self) -> None:
        ...
        self.connect(self.pwr_out, self.ic.pwr_out)
        self.connect(self.gnd, self.ic.gnd)
        self.in_cap = self.Block(DecouplingCapacitor(capacitance=1 * uFarad(tol=0.2))).connected(self.gnd, self.pwr_in)
        self.out_cap = self.Block(DecouplingCapacitor(capacitance=1 * uFarad(tol=0.2))).connected(
            self.gnd, self.pwr_out
        )
...
class Xc6206p_Device(InternalSubcircuit, LinearRegulatorDevice, GeneratorBlock, JlcPart, FootprintBlock):
    ...
    def generate(self) -> None:
        ...
        parts = [  # output range, part number, (dropout typ @10mA, max @100mA), max current, lcsc, basic part
            ...
            (
                Range.from_tolerance(3.3, 0.02),
                "XC6206P332MR",
                (0.075, 0.68),
                0.20,
                "C5446",
                True,
            ),  # basic part preferred
...
class Ap2210_Device(InternalSubcircuit, LinearRegulatorDevice, GeneratorBlock, JlcPart, FootprintBlock):
    def __init__(self, output_voltage: RangeLike):
        ...
        self.en = self.Port(
            DigitalSink(
                voltage_limits=(0, 15) * Volt, current_draw=(0.01, 25) * uAmp, input_thresholds=(0.4, 2.0) * Volt
            )
        )
...
class Lp5907_Device(
    ...
    def __init__(self, output_voltage: RangeLike):
        ...
        self.en = self.Port(
            DigitalSink(
                voltage_limits=(0, 5.5) * Volt, current_draw=(0.001, 5.5) * uAmp, input_thresholds=(0.4, 1.2) * Volt
            )
        )
    ...
    def generate(self) -> None:
        ...
        parts = [  # output voltage, Table in 6.5 tolerance varies by output voltage
            ...
            (
                Range.from_tolerance(2.5, 0.02),
                "LP5907SNX-2.5/NOPB",
                "Package_DFN_QFN:UDFN-4-1EP_1x1mm_P0.65mm_EP0.48x0.48mm",
                "C133571",
            ),
            (
                Range.from_tolerance(2.9, 0.02),
                "LP5907SNX-2.9/NOPB",
                "Package_DFN_QFN:UDFN-4-1EP_1x1mm_P0.65mm_EP0.48x0.48mm",
                "C2870726",
            ),
...
class Tlv757p_Device(
    InternalSubcircuit, LinearRegulatorDevice, PartsTablePart, JlcPart, GeneratorBlock, FootprintBlock
):
    def __init__(self, output_voltage: RangeLike):
        super().__init__()

        self.assign(self.pwr_in.voltage_limits, (1.45, 5.5) * Volt)
        self.assign(self.pwr_out.current_limits, (0, 1.2) * Amp)
        self.assign(self.actual_quiescent_current, (0.1, 40) * uAmp)  # shutdown to worst-case ground current

        self.output_voltage = self.ArgParameter(output_voltage)
        self.generator_param(self.output_voltage, self.part)

        self.en = self.Port(DigitalSink(voltage_limits=(0, 5.5) * Volt, input_thresholds=(0.3, 1) * Volt))

    @override
    def generate(self) -> None:
        super().generate()
        tolerance = 0.015  # over 125C range, Vout >= 1v
        parts = [  # output range, part, dropout voltage for non-DYD over 125C range, LCSC
            (Range.from_tolerance(1.0, tolerance), "TLV75710PDBVR", 1.2, "C2865385"),
            (Range.from_tolerance(1.2, tolerance), "TLV75712PDBVR", 1.1, "C1509296"),
            (Range.from_tolerance(1.5, tolerance), "TLV75715PDBVR", 0.85, "C2872562"),
            (Range.from_tolerance(1.8, tolerance), "TLV75718PDBVR", 0.8, "C507270"),
            (Range.from_tolerance(1.9, tolerance), "TLV75719PDBVR", 0.8, "C2868954"),
            (Range.from_tolerance(2.5, tolerance), "TLV75725PDBVR", 0.65, "C2872563"),
            (Range.from_tolerance(2.8, tolerance), "TLV75728PDBVR", 0.65, "C2863639"),
            (Range.from_tolerance(2.9, tolerance), "TLV75729PDBVR", 0.65, "C2861884"),
            (Range.from_tolerance(3.0, tolerance), "TLV75730PDBVR", 0.65, "C2872564"),
            (Range.from_tolerance(3.3, tolerance), "TLV75733PDBVR", 0.475, "C485517"),
        ]

        # TODO should prefer parts by closeness to nominal (center) specified voltage
        output_voltage_spec = self.get(self.output_voltage)
        suitable_parts = [part for part in parts if part[0] in output_voltage_spec]
        assert suitable_parts, "no regulator with compatible output"
...
class Tlv757p(VoltageRegulatorEnableWrapper, LinearRegulator):
    """1A LDO regulator in SOT-23-5 with 1.45-5.5Vin and 0.5-5Vout
    ...
    def contents(self) -> None:
        ...
        with self.implicit_connect(
            ...
            self.ic = imp.Block(Tlv757p_Device(output_voltage=self.output_voltage))
            self.in_cap = imp.Block(DecouplingCapacitor(capacitance=1 * uFarad(tol=0.2)))
            self.out_cap = imp.Block(DecouplingCapacitor(capacitance=1 * uFarad(tol=0.2)))
...
class L78l(LinearRegulator):
    ...
    def contents(self) -> None:
        ...
        self.connect(self.pwr_out, self.ic.pwr_out)
        self.connect(self.gnd, self.ic.gnd)
        self.in_cap = self.Block(DecouplingCapacitor(capacitance=0.33 * uFarad(tol=0.2))).connected(
            self.gnd, self.pwr_in
        )
        self.out_cap = self.Block(DecouplingCapacitor(capacitance=0.1 * uFarad(tol=0.2))).connected(
            self.gnd, self.pwr_out
        )
