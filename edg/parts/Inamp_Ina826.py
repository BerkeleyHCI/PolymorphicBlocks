...
class Ina826_Device(InternalSubcircuit, JlcPart, FootprintBlock):
    def __init__(self) -> None:
        ...
        analog_in_model = AnalogSink.from_supply(
            self.vsn,
            self.vsp,
            voltage_limit_tolerance=(-2, 40)
            * Volt,  # Vs- - 40 to Vs+ + 40, but down to Vs- - 2 without clamping input res
        ...
        self.ref = self.Port(
            AnalogSink.from_supply(
                self.vsn, self.vsp, signal_limit_tolerance=(0, 0) * Volt, impedance=100 * kOhm(tol=0)  # V- to V+  # typ
            )
        )
        self.out = self.Port(
