...
class Ft232hl_Device(InternalSubcircuit, FootprintBlock, JlcPart):
    def __init__(self) -> None:
        ...
        self.vregin = self.Port(
            VoltageSink(  # bus-powered: connect to VBUS
        ...
        self.vphy = self.Port(
            VoltageSink(  # connect to 3v3 through 600R 0.5A ferrite
...
class Ft232hl(Interface, GeneratorBlock):
    ...
    def __init__(self) -> None:
        ...
        self.generator_param(
            self.uart.is_connected(), self.mpsse.is_connected(), self.mpsse_cs.is_connected(), self.adbus.requested()
        )

    @override
    def contents(self) -> None:
        # connections from Figure 6.1, bus powered configuration
        self.vbus_fb = self.Block(SeriesPowerFerriteBead(hf_impedance=(600 * Ohm(tol=0.25)))).connected(
            self.pwr, self.ic.vregin
        )
        ...
        self.vphy_fb = self.Block(SeriesPowerFerriteBead(hf_impedance=(600 * Ohm(tol=0.25)))).connected(
            self.ic.vccd, self.ic.vphy
        )
        self.vphy_cap = self.Block(cap_model).connected(self.gnd, self.ic.vphy)

        self.vpll_fb = self.Block(SeriesPowerFerriteBead(hf_impedance=(600 * Ohm(tol=0.25)))).connected(
            self.ic.vccd, self.ic.vpll
        )
    ...
    def generate(self) -> None:
        ...
        self.require(
            self.uart.is_connected().implies(
                "0" not in self.get(self.adbus.requested()) and "1" not in self.get(self.adbus.requested())
            )
        )
        ...
        self.require(
            self.mpsse.is_connected().implies(
                "0" not in self.get(self.adbus.requested())
                and "1" not in self.get(self.adbus.requested())
                and "2" not in self.get(self.adbus.requested())
            )
        )
