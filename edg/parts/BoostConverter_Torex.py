...
class Xc9142_Device(InternalSubcircuit, FootprintBlock, GeneratorBlock):
    ...
    def __init__(self, output_voltage: RangeLike, frequency: RangeLike = Range.all()):
        ...
        self.ce = self.Port(
            DigitalSink.from_supply(
                self.gnd, self.vin, voltage_limit_tolerance=(0, 5) * Volt, input_threshold_abs=(0.2, 0.6) * Volt
            )
        )
    ...
    def generate(self) -> None:
        ...
        part_number_voltage, part_voltage, part_current = [
            (part, part_voltage, part_current)
            for (part, part_voltage, part_current) in self.parts_output_voltage_current
            if part_voltage in self.get(self.output_voltage)
        ][0]
        part_number_frequency, part_frequency = [
            (part, part_frequency)
            for (part, part_frequency) in self.parts_frequency
            if part_frequency in self.get(self.frequency)
        ][
            0
        ]  # lower frequency preferred, 'safer' option for compatibility
