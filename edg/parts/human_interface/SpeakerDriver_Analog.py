    ...
    def __init__(self) -> None:
        ...
        self.pwr = self.Port(
            VoltageSink(
                ...
                current_draw=(6.5, 10 + 433)
                * mAmp,  # TODO better estimate of speaker current than 1.5W into 8-ohm load
            ),
            [Power],
        )
        ...
        self.byp = self.Port(AnalogSource(voltage_out=half_vdd, signal=half_vdd))
...
class Tpa2005d1_Device(InternalSubcircuit, JlcPart, FootprintBlock):
    def __init__(self) -> None:
        ...
        input_port = AnalogSink.from_supply(
            self.gnd,
            self.pwr,
...
class Pam8302a_Device(InternalSubcircuit, JlcPart, FootprintBlock):
    def __init__(self) -> None:
        ...
        input_port = AnalogSink.from_supply(
            self.gnd, self.pwr, voltage_limit_tolerance=(-0.3, 0.3) * Volt, impedance=(142, 158) * kOhm
        )
        self.inp = self.Port(input_port)
        self.inn = self.Port(input_port)
...
class Pam8302a(SpeakerDriver, Block):
    ...
    def contents(self) -> None:
        ...
        self.pwr_cap0 = self.Block(
            DecouplingCapacitor(
                capacitance=1 * uFarad(tol=0.2),
            )
        ).connected(self.gnd, self.pwr)
        self.pwr_cap1 = self.Block(
            DecouplingCapacitor(
