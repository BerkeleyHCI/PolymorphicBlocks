...
class Drv8313_Device(InternalSubcircuit, FootprintBlock, JlcPart):
    def __init__(self) -> None:
        ...
        self.din_model = DigitalSink(  # nSleep, ENx, INx - internally pulled down 100k (Table 6.5)
            voltage_limits=(-0.3, 5.25) * Volt, input_thresholds=(0.7, 2.2)  # to input high voltage max
        )
    ...
    def contents(self) -> None:
        out_model = DigitalSource.from_supply(
            self.gnd, self.vm, current_limits=(-2.5, 2.5) * Amp  # peak current, section 1
        )
        channel_currents = []
        out_connected = []
        for i in ["1", "2", "3"]:
        ...
        self.assign(
            self.vm.current_draw, (0.5, 5) * mAmp + (0, overall_current)  # Table 6.5 Vm sleep typ to operating max
        )
