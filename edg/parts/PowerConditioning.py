...
class PriorityPowerOr(PowerConditioner, KiCadSchematicBlock, Block):
    ...
    def contents(self) -> None:
        ...
        self.fet = self.Block(
            Fet.PFet(  # doesn't account for reverse protection
    ...
    def connected_from(
        self,
        gnd: Optional[Port[GroundLink]] = None,
        pwr_hi: Optional[Port[VoltageLink]] = None,
        pwr_lo: Optional[Port[VoltageLink]] = None,
    ) -> "PriorityPowerOr":
        ...
        if pwr_hi is not None:
            builder.block().connect(pwr_hi, self.pwr_hi)
        if pwr_lo is not None:
            builder.block().connect(pwr_lo, self.pwr_lo)
        return self
...
class PmosChargerReverseProtection(PowerConditioner, KiCadSchematicBlock, Block):
    ...
    def __init__(
        ...
        r2_val: RangeLike = 100 * kOhm(tol=0.01),
    ...
    def contents(self) -> None:
        ...
        self.mp1 = self.Block(
            Fet.PFet(
                drain_voltage=batt_voltage,
                drain_current=r1_current,
...
class SoftPowerGate(PowerSwitch, KiCadSchematicBlock, Block):  # migrate from the multimater
    ...
    def contents(self) -> None:
        ...
        self.amp_res = self.Block(Resistor(resistance=self.amp_resistance))
        self.amp_fet = self.Block(
            Fet.NFet(
