...
class FetFallbackGateCharge(PartsTableSelector, BaseTableFet):
    ...
    def __init__(
        self, *args: Any, fallback_gate_charge: RangeLike = Range.from_tolerance(3000e-9, 0), **kwargs: Any
    ) -> None:
    ...
    DESCRIPTION_PARSERS: List[DescriptionParser] = [
        (
            ...
            lambda match: {
                ...
                TableFet.VGS_RATING: Range.from_abs_tolerance(0, PartParserUtil.parse_value(match.group(5), "V")),
                TableFet.VGS_DRIVE: Range(
                    PartParserUtil.parse_value(match.group(6), "V"), PartParserUtil.parse_value(match.group(5), "V")
                ),
        ...
        (
            ...
            lambda match: {
                ...
                TableFet.IDS_RATING: Range.zero_to_upper(PartParserUtil.parse_value(match.group(2), "A")),
                # Vgs isn't specified, so the Ron@Vgs is used as a lower bound; assumed symmetric
                TableFet.VGS_RATING: Range.from_abs_tolerance(0, PartParserUtil.parse_value(match.group(4), "V")),
                TableFet.VGS_DRIVE: Range(
                    PartParserUtil.parse_value(match.group(6), "V"), PartParserUtil.parse_value(match.group(4), "V")
                ),
