...
class JlcInductor(PartsTableSelectorFootprint, JlcTableSelector, TableInductor):
    ...
    PART_FOOTPRINT_PARSERS: List[DescriptionParser] = [
        (
            re.compile(r"^NR(20|24|30|40|50|60|80).*$"),
            lambda match: {
                PartsTableSelectorFootprint.KICAD_FOOTPRINT: f"Inductor_SMD:L_Taiyo-Yuden_NR-{match.group(1)}xx"
            },
        ),
        (
            re.compile(r"^SRP12(45|65)A?-.*$"),
            lambda match: {PartsTableSelectorFootprint.KICAD_FOOTPRINT: f"Inductor_SMD:L_Bourns_SRP1245A"},
        ),
        (
            re.compile(r"^SRR1015-.*$"),
            lambda match: {PartsTableSelectorFootprint.KICAD_FOOTPRINT: f"Inductor_SMD:L_Bourns-SRR1005"},
        ),
        (
            re.compile(r"^SRR1210A?-.*$"),
            lambda match: {PartsTableSelectorFootprint.KICAD_FOOTPRINT: f"Inductor_SMD:L_Bourns_SRR1210A"},
        ),
        (
            re.compile(r"^SRR1260A?-.*$"),
            lambda match: {PartsTableSelectorFootprint.KICAD_FOOTPRINT: f"Inductor_SMD:L_Bourns_SRR1260"},
        ),
        (
            re.compile(
                r"^SWPA(3010|3012|3015|4010|4012|4018|4020|4025|4030|5012|5020|5040|6020|6028|6040|6045|8040)S.*$"
            ),
            lambda match: {
        ...
        (
            re.compile(r"^SWRB(1204|1205|1207)S.*$"),
            lambda match: {
                PartsTableSelectorFootprint.KICAD_FOOTPRINT: f"Inductor_SMD:L_Sunlord_SWRB{match.group(1)}S"
            },
        ),
        (
            re.compile(r"^SLF(6025|6028|6045|7032|7045|7055|10145|12555|12565|12575)T.*$"),
            lambda match: {PartsTableSelectorFootprint.KICAD_FOOTPRINT: f"Inductor_SMD:L_TDK_SLF{match.group(1)}"},
        ),
    ...
    DESCRIPTION_PARSERS: List[DescriptionParser] = [
        (
            re.compile(r"(\S+A) (\S+H) (±\S+%)(?: \S+A)? (\S+Ω) .* Inductors.*"),
            lambda match: {
        ...
        (
            re.compile(r"(\S+A) (\S+H) ±(\S+H)(?: \S+A)? (\S+Ω) .* Inductors.*"),
            lambda match: {
                TableInductor.INDUCTANCE: Range.from_abs_tolerance(
                    PartParserUtil.parse_value(match.group(2), "H"), PartParserUtil.parse_value(match.group(3), "H")
                ),
                TableInductor.FREQUENCY_RATING: Range.all(),  # ignored, checked elsewhere
                TableInductor.CURRENT_RATING: Range.zero_to_upper(PartParserUtil.parse_value(match.group(1), "A")),
                TableInductor.DC_RESISTANCE: Range.exact(PartParserUtil.parse_value(match.group(4), "Ω")),
