...
class Mcp3561_Device(InternalSubcircuit, GeneratorBlock, FootprintBlock):
    def __init__(self, has_ext_ref: BoolLike) -> None:
        super().__init__()
        self.avdd = self.Port(
            VoltageSink(voltage_limits=(2.7, 3.6) * Volt, current_draw=(0.0004, 2.5) * mAmp)
        )  # shutdown to max operating currrent
        self.dvdd = self.Port(
            VoltageSink(
                voltage_limits=(1.8 * Volt, self.avdd.link().voltage.upper() + 0.1), current_draw=(0.002, 0.37) * mAmp
            )
        )  # shutdown to max operating current
        self.vss = self.Port(Ground())

        self.vrefp = self.Port(
            VoltageSink(  # TODO also operates in output configuration, up to 2.4v 2%
        ...
        input_model = AnalogSink.from_supply(
            self.vss,
            self.avdd,  # maximum ratings up to AVdd
...
class Mcp3561(AnalogToDigital, GeneratorBlock):
    ...
    def contents(self) -> None:
        ...
        self.avdd_res = self.Block(SeriesPowerResistor(10 * Ohm(tol=0.05))).connected(self.pwra, self.ic.avdd)
        self.dvdd_res = self.Block(SeriesPowerResistor(10 * Ohm(tol=0.05))).connected(self.pwr, self.ic.dvdd)
