...
class HiroseFh12sh(Fpc050Bottom, FootprintPassiveConnector):
    ...
    def part_footprint_mfr_name(self, length: int) -> Tuple[str, str, str]:
        return (
            f"Connector_FFC-FPC:Hirose_FH12-{length}S-0.5SH_1x{length:02d}-1MP_P0.50mm_Horizontal",
            "Hirose",
            f"FH12-{length}S-0.5SH",
        )
...
class Afc07Top(Fpc050Top, FootprintPassiveConnector, JlcPart):
    ...
    def part_footprint_mfr_name(self, length: int) -> Tuple[str, str, str]:
        ...
        self.assign(self.actual_basic_part, False)
        return (
            f"Connector_FFC-FPC:TE_{length//10}-1734839-{length%10}_1x{length:02d}-1MP_P0.5mm_Horizontal",
            "Jushuo",
...
class HiroseFh35cshw(Fpc030TopBottom, FootprintPassiveConnector, JlcPart):
    ...
    def part_footprint_mfr_name(self, length: int) -> Tuple[str, str, str]:
        ...
        self.assign(self.actual_basic_part, False)
        return (
            f"edg:Hirose_FH35C-{length}S-0.3SHW_1x{length:02d}-1MP_P0.30mm_Horizontal",
            "Hirose",
            f"FH35C-{length}S-0.3SHW",
        )
