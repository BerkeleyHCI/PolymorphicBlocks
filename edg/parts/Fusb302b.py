...
class Fusb302b_Device(InternalSubcircuit, FootprintBlock, JlcPart):
    def __init__(self) -> None:
        ...
        self.vdd = self.Port(
            VoltageSink(voltage_limits=(2.7, 5.5) * Volt, current_draw=(0.37, 40) * uAmp)
        )  # Table 11, between disabled typ and standby typ
        self.vconn = self.Port(
            VoltageSink(voltage_limits=(2.7, 5.5) * Volt, current_draw=(0, 560) * mAmp), optional=True
        )
