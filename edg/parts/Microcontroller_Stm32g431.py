...
class Stm32g431Base_Device(
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        # Power and ground
        self.pwr = self.Port(
            VoltageSink(voltage_limits=(1.71, 3.6) * Volt, current_draw=(14 * nAmp, 44.0 * mAmp)), [Power]  # Table 32
        )
    ...
    def _io_pinmap(self) -> PinMapUtil:
        ...
        io_voltage_limit = (input_range + (-0.3, 3.6) * Volt).intersect(
            self.gnd.link().voltage + (-0.3, 5.5) * Volt
        )  # Section 5.3.1
        ...
        # Pin definition, 4.10
        dio_fta_model = dio_ftca_model = dio_ftf_model = dio_ftfa_model = dio_ftfu_model = dio_ftfd_model = (
            dio_ftda_model
        ) = dio_ftu_model = dio_ft_model
        ...
        self.nrst.init_from(
            DigitalSink.from_supply(  # specified differently than other pins
                self.gnd,
                self.pwr,
        ...
        return PinMapUtil(
            [  # for 32 pins only for now
                ...
                PeripheralFixedResource("SPI2", spi_model, {"sck": ["PF1"], "miso": ["PA10"], "mosi": ["PA11"]}),
                ...
                PeripheralFixedResource(
                    "I2C1_T", i2c_target_model, {"scl": ["PA13", "PA15", "PB8"], "sda": ["PA14", "PB7", "PB9"]}
                ),
