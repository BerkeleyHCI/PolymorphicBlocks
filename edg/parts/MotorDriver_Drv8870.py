...
class Drv8870_Device(InternalSubcircuit, FootprintBlock, JlcPart):
    def __init__(self) -> None:
        ...
        self.vm = self.Port(VoltageSink.from_gnd(self.gnd, voltage_limits=(6.5, 45) * Volt, current_draw=RangeExpr()))
        self.vref = self.Port(
        ...
        dout_model = DigitalSource.from_supply(
            self.gnd, self.vm, current_limits=(-3.6, 3.6) * Amp
        )  # peak output current
        self.out1 = self.Port(dout_model)
        self.out2 = self.Port(dout_model)

        self.isen = self.Port(VoltageSink(current_draw=RangeExpr()))
    ...
    def contents(self) -> None:
        ...
        self.footprint(
            ...
            part="DRV8870DDAR",  # also compatible w/ PW package (no GND pad)
...
class Drv8870(BrushedMotorDriver):
    ...
    def contents(self) -> None:
        ...
        self.isen_res = self.Block(
            SeriesPowerResistor((1 / (10 * self.current_trip)).shrink_multiply(self.vref.link().voltage))
