...
class BlueSmirf(Interface, Radiofrequency, FootprintBlock):
    ...
    def __init__(self) -> None:
        ...
        self.pwr = self.Port(
            VoltageSink(
                voltage_limits=(3, 6)
                * Volt,  # TODO added a -10% tolerance on the low side so things still work - technically out of spec
            ),
            [Power],
        )
        self.gnd = self.Port(Ground(), [Common])
