    ...
    def __init__(self, addr_lsb: IntLike) -> None:
        ...
        self.vdd = self.Port(
            VoltageSink(
                ...
                current_draw=(0.00085, 2.5) * mAmp,
            )
        )  # quad DAC, serial inactive to EE write

        ref_model = VoltageSink(voltage_limits=self.vss.link().voltage.hull(self.vdd.link().voltage))
