...
class JlcOscillator_Device(InternalSubcircuit, Block):
    ...
    def __init__(
        self,
        in_kicad_part: StringLike,
        in_kicad_value: StringLike,
        in_kicad_datasheet: StringLike,
        in_lcsc_part: StringLike,
        in_actual_basic_part: BoolLike,
    ):
        super().__init__()
...
class Sg8101_Base_Device(JlcOscillator_Device, JlcPart, FootprintBlock):
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.vcc.init_from(
            VoltageSink(voltage_limits=(1.62, 3.62) * Volt, current_draw=(0.0003, 3.5) * mAmp)
        )  # 1.8 stdby typ to 3.3 max
        self.out.init_from(DigitalSource.from_supply(self.gnd, self.vcc, 0 * mAmp(tol=0)))
        ...
        self.footprint(
            ...
            part=self.in_kicad_part,
            value=self.in_kicad_value,
            datasheet=self.in_kicad_datasheet,
        )
...
class JlcOscillator(JlcTableSelector, TableOscillator):
    ...
    DESCRIPTION_PARSERS: List[DescriptionParser] = [
        (
            re.compile(r"(±\S+ppm) .* (\S+MHz) .* Pre-programmed Oscillators .*"),
            lambda match: {
    ...
    def _make_table(cls) -> PartsTable:
        ...
        return (
            cls._jlc_table()
            .map_new_columns(parse_row)
            .sort_by(  # TODO dedup w/ JlcTableSelector._row_sort_by
            ...
            if row[self.PART_NUMBER_COL].startswith(series):
                self.device = self.Block(
                    device_cls(
                        ...
                        row[self.DESCRIPTION_COL],
                        row[self.DATASHEET_COL],
                        row[self.LCSC_PART_HEADER],
                        row[self.BASIC_PART_HEADER] == self.BASIC_PART_VALUE,
                    )
                )
