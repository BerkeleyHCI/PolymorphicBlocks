...
class E2154fs091_Device(InternalSubcircuit, FootprintBlock):
    def __init__(self) -> None:
        ...
        self.pwr = self.Port(VoltageSink(voltage_limits=(2.4, 3.6) * Volt, current_draw=(0, 10) * mAmp), [Power])
        ...
        dio_model = DigitalBidir(
            ...
            input_thresholds=(0.3 * self.pwr.link().voltage.upper(), 0.7 * self.pwr.link().voltage.upper()),
    ...
    def contents(self) -> None:
        ...
        self.footprint(
            "U",
            "Connector_FFC-FPC:Hirose_FH12-24S-0.5SH_1x24-1MP_P0.50mm_Horizontal",  # TODO EPD outline
...
class E2154fs091(EInk):
    ...
    def contents(self) -> None:
        ...
        self.boost_sw = self.Block(
            Fet.NFet(  # TODO should use switching NFet, but we don't know anything about frequency
                drain_voltage=(0, 30) * Volt,
                drain_current=(0, 0.8) * Amp,  # assumed, from inductor rating
                gate_voltage=(3, 16)
                * Volt,  # assumed, from capacitor ratings  # TODO use pwr voltage instead of hardcoding
                rds_on=(0, 85) * mOhm,
                power=(0, 0.2)
                * Watt,  # about 4x resistive loss @ 0.8A, 85mOhm; we don't know the switch frequency or drive current
            )
        )
        self.boost_ind = self.Block(
            Inductor(inductance=10 * uHenry(tol=0.2), current=(0, 0.8) * Amp, frequency=100 * kHertz(tol=0))
        )
        self.boost_res = self.Block(
            Resistor(
                resistance=0.47 * Ohm(tol=0.01),
                power=0 * Watt(tol=0),  # TODO actual power numbers based on current draw
            )
        )
        self.boot_cap = self.Block(
            Capacitor(
        ...
        self.vdd_cap0 = self.Block(DecouplingCapacitor(capacitance=0.11 * uFarad(tol=0.2))).connected(
            self.gnd, self.pwr
        )
        self.vdd_cap1 = self.Block(DecouplingCapacitor(capacitance=1 * uFarad(tol=0.2))).connected(self.gnd, self.pwr)

        decoupling_cap_model = Capacitor(capacitance=1 * uFarad(tol=0.2), voltage=(0, 16) * Volt)
