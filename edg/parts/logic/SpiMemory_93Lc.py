...
class E93Lc_B_Device(InternalSubcircuit, GeneratorBlock, JlcPart, FootprintBlock):
    PARTS = [
        ...
        (
            2 * 1024,
            "93LC56B",
            "https://ww1.microchip.com/downloads/en/DeviceDoc/21794G.pdf",
            "C190271",
            False,
        ),  # BT-I/OT variant
    ...
    def __init__(self, size: RangeLike):
        ...
        self.spi = self.Port(SpiPeripheral(dio_model, frequency_limit=(0, 2) * MHertz))  # for Vcc >= 2.5
...
class E93Lc_B(SpiMemory):
    ...
    def contents(self) -> None:
        ...
        self.vcc_cap = self.Block(DecouplingCapacitor(capacitance=0.1 * uFarad(tol=0.2))).connected(self.gnd, self.pwr)
