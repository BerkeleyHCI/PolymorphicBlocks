    ...
    def __init__(self) -> None:
        ...
        self.vcc = self.Port(
            VoltageSink.from_gnd(
                self.com, voltage_limits=(5, 20) * Volt, current_draw=RangeExpr()  # recommended operating conditions
            )
        )
        ...
        self.vb = self.Port(VoltageSink.from_gnd(self.vs, voltage_limits=(5, 20) * Volt, current_draw=RangeExpr()))
        self.ho = self.Port(
            DigitalSource.from_supply(
                self.vs,
                self.vb,
                current_limits=(-250, 120)
