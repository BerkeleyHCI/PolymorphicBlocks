...
DescriptionParser = Tuple[re.Pattern[str], Callable[[re.Match[str]], Dict[PartsTableColumn, Any]]]
...
class JlcTableBase(PartsTableBase):
    ...
    def _jlc_table(cls) -> PartsTable:
        ...
        if JlcTableBase.__JLC_TABLE is None:  # specifically this class, so results are visible to subclasses
            JlcTableBase.__JLC_TABLE = PartsTable.from_csv_files(
                PartsTable.with_source_dir(["Pruned_JLCPCB SMT Parts Library(20220419).csv"], "resources"),
                encoding="gb2312",
            )
    ...
    def parse(description: str, regex_dictionary: Dict[str, re.Pattern[str]]) -> Dict[str, str]:
        ...
        for key, pattern in regex_dictionary.items():
            ...
            if matches:  # discard if not matched
                assert (
                    len(matches) == 1
                ), f"multiple matches for {pattern} for {description}"  # excess matches fail noisily
    ...
    @staticmethod
    def parse_full_description(
        description: str, parser_options: List[DescriptionParser]
