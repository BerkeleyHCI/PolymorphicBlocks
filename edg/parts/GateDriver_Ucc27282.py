    ...
    def __init__(self) -> None:
        ...
        self.vdd = self.Port(
            VoltageSink.from_gnd(
                self.vss, voltage_limits=(5.5, 16) * Volt, current_draw=RangeExpr()  # recommended operating conditions
            )
        )

        input_model = DigitalSink.from_supply(
            self.vss, self.vdd, voltage_limit_abs=(-5, 20) * Volt, input_threshold_abs=(0.9, 2.4) * Volt
        )
        ...
        self.lo = self.Port(
            DigitalSource.from_supply(
                self.vss, self.vdd, current_limits=(-3, 3) * Amp  # peak pullup and pulldown current
            )
        )

        self.hs = self.Port(
            Ground.from_gnd(self.vss, voltage_limits=(-8, 100))  # looser negative rating possible with pulses
        )
        self.hb = self.Port(
            VoltageSource(
        ...
        self.ho = self.Port(
            DigitalSource.from_supply(
                self.hs, self.hb, current_limits=(-3, 3) * Amp  # peak pullup and pulldown current
            )
        )
