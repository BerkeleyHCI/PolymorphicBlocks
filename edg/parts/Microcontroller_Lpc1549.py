...
class Lpc1549Base_Device(
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.pwr = self.Port(
            VoltageSink(
                ...
                current_draw=(0, 19) * mAmp
                + self.io_current_draw.upper(),  # rough guesstimate from Figure 11.1 for supply Idd (active mode)
            ),
            [Power],
        )
        ...
        self.reset = self.Port(
            DigitalSink.empty(), optional=True
        )  # internally pulled up, TODO disable-able and assignable as GPIO
    ...
    def _io_pinmap(self) -> PinMapUtil:
        ...
        adc_model = AnalogSink.from_supply(
            self.gnd,
            self.pwr,
            voltage_limit_tolerance=(0, 0),  # assumed from DIO model
...
class Lpc1549Base(
    ...
    def contents(self) -> None:
        ...
        with self.implicit_connect(ImplicitConnect(self.pwr, [Power]), ImplicitConnect(self.gnd, [Common])) as imp:
            ...
            (self.swd_pull,), _ = self.chain(self.swd_node, imp.Block(Lpc1549SwdPull()), self.ic.swd)
