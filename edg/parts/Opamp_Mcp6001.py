...
class Mcp6001_Device(InternalSubcircuit, JlcPart, FootprintBlock):
    def __init__(self) -> None:
        super().__init__()
        self.vcc = self.Port(VoltageSink(voltage_limits=(1.8, 6.0) * Volt, current_draw=(50, 170) * uAmp), [Power])
...
class Mcp6001(Opamp):
    ...
    def contents(self) -> None:
        ...
        self.vdd_cap0 = self.Block(DecouplingCapacitor(capacitance=0.1 * uFarad(tol=0.2))).connected(self.gnd, self.pwr)
