...
class Mcp4728_Device(InternalSubcircuit, FootprintBlock, GeneratorBlock, JlcPart):
    def __init__(self) -> None:
        ...
        self.vdd = self.Port(
            VoltageSink(voltage_limits=(2.7, 5.5) * Volt, current_draw=(0.040, 1400) * uAmp)
        )  # power down to all channels in normal mode

        out_model = AnalogSource.from_supply(
            self.vss,
            self.vdd,  # assumed Vref=Vdd, also configurable
