...
class Stm32l432_Device(
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.pwr = self.Port(
            VoltageSink(
                voltage_limits=(self.usb.length() > 0).then_else(
                    (3.0, 3.6) * Volt, (self.dac.length() > 0).then_else((1.8, 3.6) * Volt, (1.71, 3.6) * Volt)
                ),
        ...
        return PinMapUtil(
            [  # Table 12, partial table for up to 32-pin only
                ...
                PeripheralFixedResource(
                    "I2C3_T",
                    i2c_target_model,
                    {"scl": ["PA7"], "sda": ["PB4"]},  # TODO shared resource w/ I2C controller
                ),
                PeripheralFixedResource("I2C1", i2c_model, {"scl": ["PA9", "PB6"], "sda": ["PA10", "PB7"]}),
                PeripheralFixedResource(
                    "I2C1_T",
                    i2c_target_model,
...
class Stm32l432k(
    ...
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.generator_param(self.reset.is_connected(), self.pin_assigns, self.gpio.requested(), self.can.requested())
