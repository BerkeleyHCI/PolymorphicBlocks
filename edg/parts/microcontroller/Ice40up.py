...
class Ice40up_Device(BaseIoControllerPinmapGenerator, InternalSubcircuit, GeneratorBlock, JlcPart, FootprintBlock):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.vccio_1 = self.Port(
            VoltageSink(
                voltage_limits=(1.71, 3.46) * Volt,  # table 4.2
                current_draw=(0.0005, 9) * mAmp
                + self.io_current_draw.upper(),  # table 4.6, static to startup peak; no max given
            )
        ...
        self.creset_b = self.Port(
            DigitalSink.from_bidir(self._pio1_model)
        )  # no internal PUR, must be driven (or 10k pulled up)
        self.cdone = self.Port(
            DigitalSource.from_bidir(self._pio1_model), optional=True
        )  # dedicated on SG48, shared on UWG30
    ...
    def _io_pinmap(self) -> PinMapUtil:
        ...
        spi_model = SpiController(DigitalBidir.empty(), frequency_limit=(0, 45) * MHertz)  # user SPI, table 4.10
...
class Ice40up(Fpga, IoController):
    ...
    def contents(self) -> None:
        ...
        with self.implicit_connect(  # Vcc (core) power domain
            ImplicitConnect(self.vcc_reg.pwr_out, [Power]), ImplicitConnect(self.gnd, [Common])
        ) as imp:
        ...
        with self.implicit_connect(  # PLL power domain, section 2 of the iCE40 Hardware Checklist
            ImplicitConnect(self.pll_res.pwr_out, [Power]), ImplicitConnect(self.gnd, [Common])
        ) as imp:
