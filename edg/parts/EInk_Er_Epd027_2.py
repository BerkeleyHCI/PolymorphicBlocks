...
class Er_Epd027_2(EInk, GeneratorBlock):
    ...
    def contents(self) -> None:
        ...
        self.vsh_cap = self.Block(DecouplingCapacitor(capacitance=1 * uFarad(tol=0.2))).connected(
            self.gnd, self.device.vsh
        )
        self.vshr_cap = self.Block(DecouplingCapacitor(capacitance=1 * uFarad(tol=0.2))).connected(
            self.gnd, self.device.vshr
        )
        self.vsl_cap = self.Block(DecouplingCapacitor(capacitance=1 * uFarad(tol=0.2))).connected(
