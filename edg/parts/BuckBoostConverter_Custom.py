...
class VoltageSourceConnector(DummyDevice, NetBlock):
    ...
    def __init__(self, a_current_draw: RangeLike, b_current_draw: RangeLike) -> None:
        super().__init__()
        self.a = self.Port(
            VoltageSink(current_draw=a_current_draw), [Input]
        )  # FET top: set output voltage, allow instantaneous current draw
        self.b = self.Port(
...
class CustomSyncBuckBoostConverterPwm(DiscreteBoostConverter, Resettable):
    ...
    def contents(self) -> None:
        ...
        (self.pwr_in_force,), _ = self.chain(  # use average draw for power input draw
            self.pwr_in, self.Block(ForcedVoltageCurrentDraw(self.power_path.switch_in.current_draw)), self.buck_sw.pwr
        )
        ...
        (self.sw_out_force,), _ = self.chain(  # current draw used to size FETs, size for peak current
            ...
            self.Block(VoltageSourceConnector(Range.exact(0), self.power_path.actual_inductor_current_peak)),
            self.boost_sw.out,
        )
