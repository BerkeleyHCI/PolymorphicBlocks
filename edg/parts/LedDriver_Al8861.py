...
class Al8861_Device(InternalSubcircuit, JlcPart, FootprintBlock):
    def __init__(self, peak_output_current: FloatLike):
        ...
        self.vset_pwm = self.Port(
            DigitalSink(voltage_limits=(0, 5) * Volt, input_thresholds=(0.2, 0.25) * Volt), optional=True
        )
...
class Al8861(LedDriverPwm, LedDriverSwitchingConverter, LedDriver, GeneratorBlock):
    ...
    def generate(self) -> None:
        ...
        self.rsense = self.Block(
            CurrentSenseResistor(resistance=(1 / self.max_current).shrink_multiply(isense_ref), sense_in_reqd=False)
        )
        ...
        self.pwr_cap = self.Block(DecouplingCapacitor((4.7 * 0.8, 10 * 1.2) * uFarad)).connected(
            self.gnd, self.pwr
        ...
        self.ind = self.Block(
            Inductor(inductance=(min_inductance, float("inf")), current=peak_current, frequency=(0.25, 1) * MHertz)
        )
