...
class FlirLepton_Device(InternalSubcircuit, FootprintBlock, JlcPart):
    def __init__(self) -> None:
        ...
        self.vddio = self.Port(
            VoltageSink.from_gnd(  # IO ring and shutter assembly
                self.gnd,
        ...
        self.cci = self.Port(I2cTarget(dio_model, addresses=[0x2A]))  # frequency up to 1MHz
...
class FlirLepton(Sensor, Resettable, Block):
    ...
    def __init__(self) -> None:
        ...
        self.pwr_io = self.Export(
            self.ic.vddio, doc="3.0v IO voltage including shutter, IOs are 3.3v compatible from +0.6v tolerance rating"
        )
