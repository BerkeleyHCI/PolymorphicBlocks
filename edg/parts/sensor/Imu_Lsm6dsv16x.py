...
class Lsm6dsv16x_Device(InternalSubcircuit, FootprintBlock, JlcPart):
    def __init__(self) -> None:
        ...
        self.vddio = self.Port(VoltageSink(voltage_limits=(1.08, 3.6) * Volt))  # Extended range
        ...
        self.i2c = self.Port(I2cTarget(dio_model, addresses=[0x6A], frequency_limit=(0, 1000) * kHertz))  # for SA=0
...
class Lsm6dsv16x(Accelerometer, Gyroscope, DefaultExportBlock):
    ...
    def __init__(self) -> None:
        ...
        self.int1 = self.Export(
            self.ic.int1, optional=True, doc="Programmable interrupt. This can be configured as push-pull / open-drain."
        )
        self.int2 = self.Export(
            self.ic.int2, optional=True, doc="Programmable interrupt. This can be configured as push-pull / open-drain."
        )
