...
class Tps61040_Device(InternalSubcircuit, JlcPart, FootprintBlock):
    def __init__(self) -> None:
        ...
        self.vin = self.Port(
            VoltageSink(voltage_limits=(1.8, 6) * Volt, current_draw=(0.1, 50) * uAmp), [Power]  # quiescent current
        )
        self.en = self.Port(
            DigitalSink.from_supply(
                self.gnd, self.vin, voltage_limit_tolerance=(-0.3, 0.3), input_threshold_abs=(0.4, 1.3) * Volt
            )
        )
...
class Tps61040(VoltageRegulatorEnableWrapper, DiscreteBoostConverter):
    ...
    def contents(self) -> None:
        ...
        vin_fs_max = (vin.lower() <= vout_fs_max / 2).then_else(
            vout_fs_max / 2, (vin.lower() > vout_fs_max / 2).then_else(vin.lower(), vin.upper())
        )
        ...
        self.in_cap = self.Block(
            DecouplingCapacitor(capacitance=4.7 * uFarad(tol=0.2))  # recommended by the datasheet
        ...
        output_capacitance = (
            self.inductor.actual_inductance.upper()
            * ipeak.upper()
        ...
        self.out_cap = self.Block(
            DecouplingCapacitor(capacitance=output_capacitance)  # recommended by the datasheet
...
class Lm2733_Device(InternalSubcircuit, JlcPart, FootprintBlock):
    def __init__(self) -> None:
        ...
        self.pwr_in = self.Port(
            VoltageSink(voltage_limits=(2.7, 14) * Volt, current_draw=(0.000024, 3) * mAmp), [Power]
        )
        self.gnd = self.Port(Ground(), [Common])
        ...
        self.nshdn = self.Port(
            DigitalSink.from_supply(
                self.gnd, self.pwr_in, voltage_limit_tolerance=(-0.4, 0.3) * Volt, input_threshold_abs=(0.5, 1.5) * Volt
            )
        )
