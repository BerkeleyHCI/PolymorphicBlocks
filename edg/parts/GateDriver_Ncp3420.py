...
class Ncp3420_Device(InternalSubcircuit, JlcPart, FootprintBlock):
    def __init__(self) -> None:
        ...
        input_model = DigitalSink.from_supply(
            self.pgnd, self.vcc, voltage_limit_abs=(-0.3, 6.5) * Volt, input_threshold_abs=(0.8, 2.0) * Volt
        )
        ...
        self.swn = self.Port(
            Ground.from_gnd(self.pgnd, voltage_limits=(-5, 35))  # no current draw since this is a "ground" pin
        )
        self.bst = self.Port(
            VoltageSink.from_gnd(
                self.swn,
    ...
    def generate(self) -> None:
        ...
        if self.get(self.high_pwr.is_connected()):
            ...
            if self.get(self.has_boot_diode):
                self.boot = self.Block(
                    Diode(self.high_gnd.link().voltage + self.pwr.link().voltage, self.ic.vcc.current_draw)
                )
                self.connect(
