...
class Drv8833_Device(InternalSubcircuit, FootprintBlock, JlcPart):
    def __init__(self) -> None:
        super().__init__()
        self.vm = self.Port(VoltageSink(voltage_limits=(2.7, 10.8) * Volt, current_draw=RangeExpr()))
        ...
        self.nsleep = self.Port(
            DigitalSink(  # internally pulled down (sleep mode)
    ...
    def contents(self) -> None:
        self.assign(
            self.vm.current_draw,
            (1.6, 3000) * uAmp  # from sleep to max operating
            + (
        ...
        self.require(
            self.aout1.is_connected()
            | self.aout2.is_connected()
            | self.bout1.is_connected()
            | self.bout2.is_connected()
        )
