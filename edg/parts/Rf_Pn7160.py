...
class NfcAntenna(FootprintBlock, GeneratorBlock, Interface):
    ...
    @classmethod
    def impedance_from_lrc(
        cls, freq: float, inductance: float, resistance: float, capacitance: float = 0.1e-9
    ) -> complex:
        ...
        imagpart = (
            w * inductance - (w**3) * (inductance**2) * capacitance - w * (resistance**2) * capacitance
        ) / ((1 - (w**2) * inductance * capacitance) ** 2 + (w * resistance * capacitance) ** 2)
        return complex(realpart, imagpart)

    def __init__(
        self,
        ant_footprint: StringLike,
        freq: FloatLike,
    ...
    def generate(self) -> None:
        ...
        impedance = NfcAntenna.impedance_from_lrc(
            self.get(self.freq), self.get(self.inductance), self.get(self.resistance), self.get(self.capacitance)
        )
...
class NfcAntennaDampening(InternalSubcircuit, GeneratorBlock):
    ...
    def generate(self) -> None:
        ...
        res_value = self.damp_res_from_impedance(
            complex(self.get(self.ant_r), self.get(self.ant_x)), self.get(self.target_q)
        )
...
class DifferentialLcLowpassFilter(GeneratorBlock, RfFilter):
    ...
    def generate(self) -> None:
        ...
        inductor_model = Inductor(
            self.get(self.inductance) * Henry(tol=0.1), current=self.current, frequency=(0, self.freq_cutoff)
        )
        ...
        cap_model = Capacitor(capacitance * Farad(tol=0.2), voltage=self.voltage)
...
class DifferentialLLowPassFilter(GeneratorBlock, RfFilter):
    ...
    def generate(self) -> None:
        ...
        diff_cs, diff_cp = self._calculate_se_values(
            self.get(self.freq),
            complex(self.get(self.src_r), self.get(self.src_x)),
            complex(self.get(self.snk_r), self.get(self.snk_x)),
        )
        cs_model = Capacitor(-diff_cs * 2 * Farad(tol=0.2), voltage=self.voltage)
        self.cs1 = self.Block(cs_model)
        self.cs2 = self.Block(cs_model)
        cp_model = Capacitor(-diff_cp * 2 * Farad(tol=0.2), voltage=self.voltage)
...
class Pn7160_Device(InternalSubcircuit, FootprintBlock, JlcPart):
    def __init__(self) -> None:
        ...
        self.vbat = self.Port(
            VoltageSink.from_gnd(self.vss, voltage_limits=(2.5, 5.5) * Volt, current_draw=(0.0105, 290) * mAmp)
        )  # hard power down to continuous transmit, limit of 330mA
        self.vddup = self.Port(VoltageSink.from_gnd(self.vss, voltage_limits=(2.8, 5.8) * Volt))
        self.vddpad = self.Port(
            VoltageSink.from_gnd(self.vss, voltage_limits=(3.0, 3.6) * Volt)
        )  # also available in 1.8v nominal
        ...
        self.ven = self.Port(
            DigitalSink.from_supply(  # reset
                self.vss, self.vbat, voltage_limit_tolerance=(0, 0), input_threshold_abs=(0.4, 1.1) * Volt
            )
        )
...
class Pn7160(Resettable, DiscreteRfWarning, Block):
    ...
    def contents(self) -> None:
        ...
        with self.implicit_connect(
            ...
            self.cvddmid = imp.Block(DecouplingCapacitor(capacitance=100 * nFarad(tol=0.1))).connected(
                pwr=self.ic.vddmid
            )
            ...
            self.rx = self.Block(
                Pn7160RxFilter(resistance=2.2 * kOhm(tol=0.05), capacitance=1 * nFarad(tol=0.1), voltage=CAP_VOLTAGE)
            )
            self.connect(self.ic.rxp, self.rx.out1)
            self.connect(self.ic.rxn, self.rx.out2)

            self.emc = imp.Block(
                DifferentialLcLowpassFilter(
                    freq_cutoff=14.7 * MHertz,
                    inductance=220 * nHenry,
                    input_res=20 * Ohm,
                    freq=SIGNAL_FREQ,
                    current=(0, 300) * mAmp,
                    voltage=CAP_VOLTAGE,
            ...
            self.ant = self.Block(
                NfcAntenna(
                    ant_footprint="board:an13219",
                    freq=SIGNAL_FREQ,
                    inductance=1522 * nHenry,  # from NXP AN13219 PCB antenna
                    resistance=1.40 * Ohm,
                    capacitance=2.0 * pFarad,
                )
            )
            ...
            self.match = imp.Block(
                DifferentialLLowPassFilter(  # complex conjugate both sides
                    freq=SIGNAL_FREQ,
                    src_r=self.emc.z_real,
                    src_x=-self.emc.z_imag,
                    snk_r=self.damp.z_real,
                    snk_x=-self.damp.z_imag,
                    voltage=CAP_VOLTAGE,
                )
            )
