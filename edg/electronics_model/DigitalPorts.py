...
class DigitalLink(CircuitLink):
    ...
    def contents(self) -> None:
        ...
        self.description = DescriptionString(
            ...
            DescriptionString.FormatUnits(self.output_thresholds, "V"),
            ", <b>input thresholds</b>: ",
            DescriptionString.FormatUnits(self.input_thresholds, "V"),
        )
        ...
        self.assign(
            self.voltage_limits,
            self.sinks.intersection(lambda x: x.voltage_limits).intersect(
                self.bidirs.intersection(lambda x: x.voltage_limits)
            ),
        )
        ...
        self.assign(
            self.current_limits,
            self.sources.intersection(lambda x: x.current_limits).intersect(
        ...
        self.assign(
            ...
            self.sources.intersection(lambda x: x.output_thresholds).intersect(
        ...
        self.require(self.sources.any_connected() | (self.bidirs.length() > 0), "requires connected source or bidir")
        ...
        self.assign(
            self._has_low_signal_driver,  # assumed bidirs are true directional drivers
            self.bidirs.any_connected() | self.sources.any(lambda x: x.low_driver),
        )
        self.assign(
            self._has_high_signal_driver, self.bidirs.any_connected() | self.sources.any(lambda x: x.high_driver)
        )

        is_bridged_internal = (
            self.sources.any(lambda x: x._bridged_internal)
            | self.sinks.any(lambda x: x._bridged_internal)
            | self.bidirs.any(lambda x: x._bridged_internal)
        )
        self.require(
            is_bridged_internal
            | self._has_high_signal_driver.implies(self._has_low_signal_driver | self.pulldown_capable),
            "requires low driver or pulldown",
        )
        self.require(
            is_bridged_internal
            | self._has_low_signal_driver.implies(self._has_high_signal_driver | self.pullup_capable),
            "requires high driver or pullup",
        )

        # when multiple sources, ensure they all drive only one signal direction (eg, open drain)
        self.require(
            (self.sources.count(lambda x: x.high_driver) > 1).implies(~self.sources.any(lambda x: x.low_driver))
...
class DigitalSinkBridge(CircuitPortBridge):
    def __init__(self) -> None:
        ...
        self.outer_port = self.Port(
            DigitalSink(voltage_limits=RangeExpr(), current_draw=RangeExpr(), input_thresholds=RangeExpr())
        )
...
class DigitalSource(DigitalBase):
    ...
    def from_supply(
        ...
        *,
        output_threshold_offset: Optional[Tuple[FloatLike, FloatLike]] = None,
    ) -> DigitalSource:
        supply_range = VoltageLink._supply_voltage_range(neg, pos)
        if output_threshold_offset is not None:
            ...
            output_threshold = (
                GroundLink._voltage_range(neg).lower() + output_offset_low,
    ...
    def from_supply(
        ...
        *,
        ...
        if input_threshold_factor is not None:
            ...
            input_threshold = (
                RangeExpr._to_expr_type(neg_base) + RangeExpr._to_expr_type(input_range) * input_threshold_factor
            )
        elif input_threshold_abs is not None:
            assert input_threshold_factor is None, "can only specify one input threshold type"
            input_threshold = RangeExpr._to_expr_type(input_threshold_abs)  # TODO avoid internal functions?
        ...
        if output_threshold_factor is not None:
            ...
            output_threshold = RangeExpr._to_expr_type(neg_base) + output_threshold_factor * RangeExpr._to_expr_type(
                output_range
            )
