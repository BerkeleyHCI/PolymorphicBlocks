...
class KiCadSchematicParserTest(unittest.TestCase):
    ...
    def test_kicad_power(self) -> None:
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "resources", "test_kicad_import_power.kicad_sch"), "r"
        ) as file:
            file_data = file.read()
        sch = KiCadSchematic(file_data)
        nets = [net_to_tuple(x) for x in sch.nets]
    ...
    def test_degenerate_label(self) -> None:
        with open(
            os.path.join(os.path.dirname(__file__), "resources", "test_kicad_import_badlabel.kicad_sch"), "r"
        ) as file:
            file_data = file.read()
    ...
    def test_connectedports(self) -> None:
        ...
        with open(
            os.path.join(os.path.dirname(__file__), "resources", "test_kicad_import_connectedports.kicad_sch"), "r"
        ) as file:
            file_data = file.read()
        sch = KiCadSchematic(file_data)
        nets = [net_to_tuple(x) for x in sch.nets]
