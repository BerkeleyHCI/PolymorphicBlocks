...
class KiCadLibSymbol:
    ...
    def __init__(self, sexp: List[Any]):
        ...
        self.properties: Dict[str, str] = {
            test_cast(prop[1], str): test_cast(prop[2], str) for prop in sexp_dict.get("property", [])
        }
        symbol_elts = itertools.chain(
            *[symbol_sexp[2:] for symbol_sexp in sexp_dict.get("symbol", [])]  # discard 'symbol' and the name
        )
...
class KiCadSchematic:
    ...
    def __init__(self, data: str, order: SchematicOrder = SchematicOrder.xy):
        ...
        self.lib_symbols = {
            symbol.name: symbol
            for symbol in [KiCadLibSymbol(elt) for elt in extract_only(sexp_dict.get("lib_symbols", []))[1:]]
        }  # discard car
        ...
        symbol_pins = list(
            itertools.chain(
                *[[KiCadPin(symbol, pin) for pin in self.lib_symbols[symbol.lib_ref].pins] for symbol in self.symbols]
            )
        )
        power_pins = list(
            itertools.chain(
                *[[KiCadPin(symbol, pin) for pin in self.lib_symbols[symbol.lib_ref].pins] for symbol in power_symbols]
            )
        )
        labels.extend([KiCadPowerLabel(power_pin.symbol.properties["Value"], power_pin.pt) for power_pin in power_pins])
        ...
        labels_by_name: Dict[str, List[Union[KiCadPin, KiCadTunnel, KiCadMarker]]] = (
            {}
        )  # it will not be a KiCadPin but needed for invariant typing
        ...
        nets_elts: List[List[Union[KiCadPin, KiCadTunnel, KiCadMarker]]] = [  # transform points into KiCad elements
            self._deduplicate_list(itertools.chain(*[elts_by_point.get(point, []) for point in points]))
