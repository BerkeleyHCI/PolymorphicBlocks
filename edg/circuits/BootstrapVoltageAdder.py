    ...
    def contents(self) -> None:
        ...
        self.import_kicad(
            ...
            conversions={
                ...
                "pwr": VoltageSink(),
                "pwr_pos": VoltageSink(current_draw=out_current),
                "pwr_neg": VoltageSink(current_draw=out_current),
                "pwm": DigitalSink(current_draw=(-out_current.upper(), out_current.upper())),
                "out_pos": VoltageSource(
                    self.pwr_pos.link().voltage + self.pwm.link().output_thresholds.upper(),
                    # NOTE - assumes flying cap = bulk cap
                    current_limits=(
                        0,
                        (self.c_pos.actual_capacitance * self.ripple_limit / 2 * self.frequency).lower(),
                    ),
                ),
                "out_neg": VoltageSource(
                    ...
                    current_limits=(
                        0,
                        (self.c_neg.actual_capacitance * self.ripple_limit / 2 * self.frequency).lower(),
                    ),
                ),
            },
        )
