...
class DigitalIsolator(Block):
  ...
  def __init__(self):
      ...
      self.in_a = self.Port(Vector(DigitalSink.empty()))
      self.out_a = self.Port(Vector(DigitalSource.empty()))

      self.pwr_b = self.Port(VoltageSink.empty())
      self.gnd_b = self.Port(Ground.empty())
      self.in_b = self.Port(Vector(DigitalSink.empty()))
      self.out_b = self.Port(Vector(DigitalSource.empty()))
