...
class CompilerLinkArrayExpansionTest extends AnyFlatSpec with CompilerTestUtil {
  val library = Library(
    ...
    blocks = Seq(
      Block.Block(
        "sourceBlock", // array elements source
        ports = SeqMap(
      ...
      Block.Block(
        "elasticSinkBlock", // array elements sink
        ports = SeqMap(
        ...
        constraints = SeqMap(
          ...
          "elements" -> Constraint.Assign(
            ref.LocalPath(Seq(
              ref.LocalStep(ref.LocalStep.Step.Name("port")),
              ref.LocalStep(ref.LocalStep.Step.ReservedParam(ref.Reserved.ELEMENTS))
            )),
            ValueExpr.Ref(ref.LocalPath(Seq(
              ref.LocalStep(ref.LocalStep.Step.Name("port")),
  ...
  "Compiler on design with link-array" should "expand constraints and link arrays" in {
    ...
    Seq("0", "1").foreach { sinkIndex =>
      Map("a" -> 11, "b" -> 12, "c" -> 13).foreach { case (elementIndex, paramValue) =>
        compiler.getValue(IndirectDesignPath() + "link" + "sinks" + sinkIndex + elementIndex + "param") should
          equal(Some(IntValue(paramValue)))
        compiler.getValue(
          IndirectDesignPath() + "link" + "sinks" + sinkIndex + elementIndex + IndirectStep.IsConnected
        ) should
          equal(Some(BooleanValue(true)))
        compiler.getValue(IndirectDesignPath() + "link" + elementIndex + "sinks" + sinkIndex + "param") should
          equal(Some(IntValue(paramValue)))
        compiler.getValue(
          IndirectDesignPath() + "link" + elementIndex + "sinks" + sinkIndex + IndirectStep.IsConnected
        ) should
          equal(Some(BooleanValue(true)))
      }
    }
    ...
    Seq("a", "b", "c").foreach { elementIndex =>
      compiler.getValue(
        IndirectDesignPath() + "source" + "port" + elementIndex + IndirectStep.ConnectedLink + "param"
      ) should
    ...
    Seq("0", "1").foreach { sinkIndex =>
      Seq("a", "b", "c").foreach { elementIndex =>
        compiler.getValue(
          IndirectDesignPath() + s"sink$sinkIndex" + "port" + elementIndex + IndirectStep.ConnectedLink + "param"
        ) should
          equal(Some(IntValue(-1)))
        compiler.getValue(
          IndirectDesignPath() + s"sink$sinkIndex" + "port" + elementIndex + IndirectStep.IsConnected
        ) should
          equal(Some(BooleanValue(true)))
      }
    }
  ...
  "Compiler on design with partially-connected link-arrays, sinks only" should "work" in {
    ...
    Seq("a", "b", "c").foreach { elementIndex =>
      compiler.getValue(
        IndirectDesignPath() + "sink" + "port" + elementIndex + IndirectStep.ConnectedLink + "param"
      ) should
        equal(Some(IntValue(-1)))
