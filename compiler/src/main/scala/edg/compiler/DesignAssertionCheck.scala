...
/** Evaluates all assertions and returns those that aren't valid or are missing parameters
  */
class DesignAssertionCheck(compiler: Compiler)
    extends DesignMap[Unit, Seq[CompilerError.AssertionError], Seq[CompilerError.AssertionError]] {
  def mapConstraint(
  ...
  override def mapBlock(
      ...
      links: SeqMap[String, Seq[CompilerError.AssertionError]]
  ): Seq[CompilerError.AssertionError] = {
