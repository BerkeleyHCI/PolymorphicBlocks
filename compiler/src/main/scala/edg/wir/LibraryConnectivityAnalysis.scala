...
/** Provides connectivity analysis (connectable ports) on libraries. Analysis is lazy, but the ports and links maps are
  * gathered eagerly, so state is consistent with the library as of creation time of this object.
  */
class LibraryConnectivityAnalysis(library: Library) {
  ...
  lazy private val bridgedPortByOuterMap: Map[ref.LibraryPath, (ref.LibraryPath, ref.LibraryPath)] = allBlocks.toSeq
    .collect {
      case (blockType, block) // filter by PortBridge class
    ...
    .collect {
      case (srcPortType, pairs) if pairs.length == 1 => // discard overlaps
        srcPortType -> pairs.map(_._2).head
    }

  /** Returns the link associated with a port, or None if it failed (eg, port not in library, no associated links).
    */
  def linkOfPort(port: ref.LibraryPath): Option[ref.LibraryPath] = {
    portToLinkMap.get(port)
  }

  /** Returns all the port types and counts (0 = infinity) that can be connected to this link. Returns an empty map if
    * the link is invalid
    */
