...
class Xiao_Rp2040(IoControllerUsbOut, IoControllerPowerOut, Rp2040_Ios, IoController, GeneratorBlock, FootprintBlock):
  ...
  def __init__(self, **kwargs) -> None:
    ...
    self.pwr.init_from(VoltageSink(
      voltage_limits=(2.97, 3.63)*Volt,  # stricter limits for non-compromised ADC performance
      current_draw=28.9*mAmp(tol=0) + self.io_current_draw.upper()  # "normal" power consumption
    ))
    self.gnd.init_from(Ground())

    self.gnd_out.init_from(GroundSource())
    self.vusb_out.init_from(VoltageSource(
      voltage_out=UsbConnector.USB2_VOLTAGE_RANGE,  # assumed
      current_limits=UsbConnector.USB2_CURRENT_LIMITS  # assumed
    ))
    self.pwr_out.init_from(VoltageSource(
      voltage_out=3.3*Volt(tol=0),  # assumed
      current_limits=RangeExpr.ALL  # unspecified
    ))
  ...
  def generate(self) -> None:
    ...
    self.footprint(
      ...
      datasheet='https://files.seeedstudio.com/wiki/XIAO/Seeed-Studio-XIAO-Series-SOM-Datasheet.pdf'
    )
