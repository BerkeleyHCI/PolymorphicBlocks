...
class JLCPCB_ResistorTable(ESeriesResistor):
  ...
  def _generate_table(cls) -> PartsTable:
    RESISTOR_MATCHES = {
      ...
      'tolerance': "\s?([\u00B1]\d+(\.\d*)?%)\s",
      'power_dissipation': "\s?(\d+(\.\d*)?[GMkmunp]?)?W\s",
    }
      ...
      if (row['Library Type'] != 'Basic' and row['First Category'] != 'Resistors'):
        ...
        try:
          ...
          footprint = (cls.PACKAGE_FOOTPRINT_MAP.get(row['Package / Case'], None) or
                       ...
                       cls.MPN_NR_FOOTPRINT_REGEX.apply(row['MFR.Part']))
          ...
          new_rows[cls.POWER_DISSIPATION] = PartsTableUtil.parse_value(extracted_values['power_dissipation'][0], 'W')
...
class JLCPCB_Resistor(Resistor, FootprintBlock, GeneratorBlock):
   ...
   def select_inductor(self, resistance: Range, power_dissipation: Range,
     ...
     compatible_parts = JLCPCB_ResistorTable.table().filter(lambda row: (
         (not part_spec or part_spec == row['MFR.Part']) and
         ...
         row[JLCPCB_ResistorTable.POWER_DISSIPATION].fuzzy_in(power_dissipation)
     ))
     ...
     self.footprint(
       ...
       mfr=part['Manufacturer'], part=part['MFR.Part'],
