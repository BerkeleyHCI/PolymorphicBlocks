...
class TableDeratingCapacitor(Capacitor):
  ...
  def select_capacitor(self, capacitance: Range, voltage: Range,
                       ...
                       part_spec: str, footprint_spec: str, Table: Type) -> None:
    # Pre-filter out by the static parameters
    # Note that we can't filter out capacitance before derating
    prefiltered_parts = Table.table().filter(lambda row: (
      (not part_spec or part_spec == row[Table.PART_NUMBER]) and
      (not footprint_spec or footprint_spec == row[Table.FOOTPRINT]) and
      voltage.fuzzy_in(row[Table.VOLTAGE_RATING]) and
      Range.exact(row[Table.NOMINAL_CAPACITANCE]).fuzzy_in(single_nominal_capacitance)
    ))
    ...
    self.assign(self.selected_voltage_rating, part[Table.VOLTAGE_RATING])
    self.assign(self.selected_capacitance, part[Table.CAPACITANCE])
    self.assign(self.selected_derated_capacitance, part[self.DERATED_CAPACITANCE])

    self.footprint(
      'C', part[Table.FOOTPRINT],
      {
        '1': self.pos,
        '2': self.neg,
      },
      mfr=part[Table.MANUFACTURER], part=part[Table.PART_NUMBER],
      value=f"{part['Capacitance']}, {part['Voltage - Rated']}",
      datasheet=part[Table.DATASHEETS]
      )
      ...
      def parallel_row(row: PartsTableRow) -> Optional[Dict[PartsTableColumn, Any]]:
        ...
        cap_model = DummyCapacitor(capacitance=part[Table.NOMINAL_CAPACITANCE],
                                   voltage=self.voltage,
                                   footprint=part[Table.FOOTPRINT],
                                   manufacturer=part[Table.MANUFACTURER], part_number=part[Table.PART_NUMBER],
                                   value=f"{part['Capacitance']}, {part['Voltage - Rated']}")
