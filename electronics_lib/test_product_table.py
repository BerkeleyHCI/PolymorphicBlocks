...
class ProductTableTest(unittest.TestCase):
  def setUp(self) -> None:
    path = os.path.join(os.path.dirname(__file__), 'resources', 'test_table.csv')
    with open(path, newline='') as csvfile:
      reader = csv.reader(csvfile)
      self.table = ProductTable(next(reader), [row for row in reader])

  def test_product_table(self) -> None:
    self.assertEqual(self.table.header, ['header1', 'header2', 'header3'])
  ...
  def test_sum(self) -> None:
    ...
    self.assertEqual(table.header, ['header1', 'header2', 'header3'])
