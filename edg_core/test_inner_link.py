...
class InnerLinkTestCase(unittest.TestCase):
  ...
  def test_inner_links(self) -> None:
    self.assertEqual(self.pb.links['a_net'].lib_elem.target.name, "edg_core.test_common.TestLink")
    self.assertEqual(self.pb.links['b_net'].lib_elem.target.name, "edg_core.test_common.TestLink")
...
class TestBundleSourceBlock(Block):
  def __init__(self):
    super().__init__()
    self.source = self.Port(TestBundleSource())


class TestBundleSinkBlock(Block):
  def __init__(self):
    super().__init__()
    self.sink = self.Port(TestBundleSink())


class TestBundleTopBlock(Block):
  def contents(self):
    super().contents()
    self.source = self.Block(TestBundleSourceBlock())
    self.sink1 = self.Block(TestBundleSinkBlock())
    self.sink2 = self.Block(TestBundleSinkBlock())
    self.net = self.connect(self.source.source, self.sink1.sink, self.sink2.sink)


class TestBundleConnectTestCase(unittest.TestCase):
  def setUp(self) -> None:
    driver = Driver([test_common, sys.modules[__name__]])
    design = Driver.elaborate_toplevel(TestBundleTopBlock())
    self.pb = InstantiationTransform(driver.libs()).transform_design(design).contents

  def test_link_instantiation(self) -> None:
    self.assertEqual(self.pb.links['net'].link.superclasses[0].target.name, "edg_core.test_inner_link.TestBundleLink")

    self.assertEqual(self.pb.links['net'].link.links['a_net'].link.superclasses[0].target.name, "edg_core.test_common.TestLink")
    self.assertEqual(self.pb.links['net'].link.links['a_net'].link.superclasses[0].target.name, "edg_core.test_common.TestLink")

  def test_export_expansion(self):
    self.assertEqual(self.pb.links['net'].link.superclasses[0].target.name,
                     'edg_core.test_inner_link.TestBundleLink')
    ...
    expected_conn.exported.internal_block_port.ref.steps.add().name = 'a_net'
    expected_conn.exported.internal_block_port.ref.steps.add().name = 'source'
    self.assertIn(expected_conn, self.pb.links['net'].link.constraints.values())

    expected_conn = edgir.ValueExpr()
    expected_conn.exported.exterior_port.ref.steps.add().name = 'sinks'
    expected_conn.exported.exterior_port.ref.steps.add().name = '0'
    expected_conn.exported.exterior_port.ref.steps.add().name = 'a'
    expected_conn.exported.internal_block_port.ref.steps.add().name = 'a_net'
    expected_conn.exported.internal_block_port.ref.steps.add().name = 'sinks'
    expected_conn.exported.internal_block_port.ref.steps.add().name = '0'
    self.assertIn(expected_conn, self.pb.links['net'].link.constraints.values())
    ...
    expected_conn.exported.internal_block_port.ref.steps.add().name = 'a_net'
    expected_conn.exported.internal_block_port.ref.steps.add().name = 'sinks'
    expected_conn.exported.internal_block_port.ref.steps.add().name = '1'
    self.assertIn(expected_conn, self.pb.links['net'].link.constraints.values())
